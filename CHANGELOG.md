...
- 分析后再次检查 HIR 节点资源上限，防止默认参数物化绕过前置门禁；新增 HIR/semantic/name/flow/alias-effect dense/revision/stale、scope corruption、弱化 effect、跨函数摘要、call borrow/copy/forward/lifetime/overlap，以及目标 ABI/temporary/scope/declaration/module/translation-unit/expression/statement representation plan 负向测试，内部测试增至 156 项；本轮生产代码行覆盖率实测 89.01%（17502/19663），继续高于 85% 门槛。
