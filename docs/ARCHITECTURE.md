...
本文描述 0.5.4 当前源码树的实际架构。CMake 同时固定 C17/C++17 标准基线，但当前生产实现和公共 API 使用 C++17；`cpp` 是 C++ 输出目标的代码身份，C++17 是当前生成标准。项目仍处于 0.x 开发期，只维护这里描述的精确当前 contract，不提供旧 MPF 兼容层。最终职责、性能模型和后续迁移验收条件见 [商业级编译器管线方案](COMPILER_PIPELINE.md)。
...
Matlab 方阵除法由 Semantic v11 的 `MatrixStructurePolicy` 明确选择运行时结构检测，MIR v16 与两个目标 LIR v22 原样传播并逐层验证；renderer 不读取 shape 或 helper 名称推导算法。JavaScript 与 `cpp` runtime 各自只求值一次系数矩阵，按精确零模式分类为 diagonal、upper-triangular、lower-triangular 或 dense：对角路径直接逐元素除法，三角路径使用前向/回代及对应转置求解，dense 路径回退到部分主元 LU。右除通过转置后的同一结构分派保持左右语义一致；每条路径使用与自身求解器一致的 1-范数倒条件估计和同一 square-warning policy。当前不把 tridiagonal、symmetric/Hermitian、positive-definite、sparse、complex 或容差结构识别冒充为已完成能力。
