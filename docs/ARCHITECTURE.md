...
Matlab array value semantics 不能退化为 JavaScript object alias。JavaScript LIR v43 以 `AssignmentValueForm::copy_array` 标记 dense array name-to-name assignment，以 `ParameterPassing::value_copy` 标记会被索引修改或具有已知 dense-array 类型的 local-function 形参；runtime 只在这些共享边界复制一次并保留显式 shape descriptor，fresh array 的每次写入不重复深拷贝。Sparse CSC 则采用 immutable-update contract：`MutationOwnership::replace_with_result` 要求 indexed assignment/growth/deletion 先构造并验证新 CSC，再替换目标根值，旧 alias 保持不变。C++17 的值容器原生满足相同可观察语义；Python/TypeScript 引用语义和 Fortran 可写引用 ABI 不经过这条 Matlab copy plan。
...
canonical logical CSC，全维结果产生 full logical scalar。JavaScript 与 `cpp` LIR v43 分别把
