...
本文描述 0.4.4 发布版的实际架构。CMake 同时固定 C17/C++17 标准基线，但当前生产实现和公共 API 使用 C++17；`cpp` 是 C++ 输出目标的代码身份，C++17 是当前生成标准。最终职责、性能模型和后续迁移验收条件见 [商业级编译器管线方案](COMPILER_PIPELINE.md)。
...
0.4.4 在 0.4.3 静态 N 维 storage-region 主链路上补齐指令内存语义：semantic v2、MIR v6 和 alias/effect v3 以同一规范化事实贯通 Analyzer、expression/call contract、直接 memory operation、跨函数参数实例化、verifier 与确定性 dump。copy-in 与 writeback 使用同一个原 actual region，copy-out 不伪造读取；优化紧凑化不会让访问事实与 `InstructionId` 错位。访问冲突查询使用 region 精化同根 alias，无法证明时不乐观降级。优化仍保持明确的保守边界；全局值编号、一般 phi/SCCP、浮点代数、loop/memory SSA、跨函数优化和 region-aware DCE 均未宣称完成。独立 target AST、一般 RAII/copy-move/runtime ABI node、完整四语言官方 grammar、动态 rank/广播、跨动态 extent/一般 view/pointer 的完整区域组合与稳定插件 ABI 仍未完成。所有边界逐项记录在 [TODO](../TODO.md)。
