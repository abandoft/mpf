...
本文描述 0.4.7 当前源码树的实际架构。CMake 同时固定 C17/C++17 标准基线，但当前生产实现和公共 API 使用 C++17；`cpp` 是 C++ 输出目标的代码身份，C++17 是当前生成标准。项目仍处于 0.x 开发期，只维护这里描述的精确当前 contract，不提供旧 MPF 兼容层。最终职责、性能模型和后续迁移验收条件见 [商业级编译器管线方案](COMPILER_PIPELINE.md)。
...
- `BinaryOperator` 是普通与 Matlab 逐元素二元运算的规范身份，随语言 AST、HIR、MIR `OperationAttributeTable` 和两个目标 LIR 传递；源 spelling 只用于诊断/调试。Analyzer、MIR lazy CFG/constant folding 和 target representation 不从 renderer 中反向猜测操作。Matlab 二维矩阵乘法与同 shape/scalar 逐元素运算由两个目标各自的 `matlab_array_operation` plan 选择 runtime helper；不相容 shape 和未实现矩阵运算在 target lowering 前失败关闭；
