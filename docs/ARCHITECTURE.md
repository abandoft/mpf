...
Matlab array value semantics 不能退化为 JavaScript object alias。JavaScript LIR v40 以 `AssignmentValueForm::copy_array` 标记 dense array name-to-name assignment，以 `ParameterPassing::value_copy` 标记会被索引修改或具有已知 dense-array 类型的 local-function 形参；runtime 只在这些共享边界复制一次并保留显式 shape descriptor，fresh array 的每次写入不重复深拷贝。Sparse CSC 则采用 immutable-update contract：`MutationOwnership::replace_with_result` 要求 indexed assignment/growth/deletion 先构造并验证新 CSC，再替换目标根值，旧 alias 保持不变。C++17 的值容器原生满足相同可观察语义；Python/TypeScript 引用语义和 Fortran 可写引用 ABI 不经过这条 Matlab copy plan。
...
`SparseArithmeticPlan` 与一般 dense broadcast、矩阵乘法及 `.*` 计划彼此独立，固定 `+`/`-` operation、static compatible-size 左右/结果 shape、逐轴 broadcast identity、operand/result storage 与 preserve-sparse/materialize-dense policy。两侧均为 CSC 时，JavaScript 与 `cpp` 分别按结果列归并 canonical CSC entry，处理 singleton row/column expansion、exact-zero cancellation 和 zero extent，结果为 Matlab double CSC；一侧为 dense 或 scalar 时只物化 dense 结果，不物化 sparse source。五个 operation/policy/storage integer 与三组 shape 组成目标专属 ABI；renderer 只序列化 representation planner 已验证的 helper 与实参。`sparse_arithmetic` 是依赖 `sparse_matrices` 的可选 runtime fragment，未出现稀疏加减法时不会进入输出。
...
canonical logical CSC，全维结果产生 full logical scalar。JavaScript 与 `cpp` LIR v40 分别把
