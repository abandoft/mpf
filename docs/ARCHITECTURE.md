...
本文描述 0.4.8 当前源码树的实际架构。CMake 同时固定 C17/C++17 标准基线，但当前生产实现和公共 API 使用 C++17；`cpp` 是 C++ 输出目标的代码身份，C++17 是当前生成标准。项目仍处于 0.x 开发期，只维护这里描述的精确当前 contract，不提供旧 MPF 兼容层。最终职责、性能模型和后续迁移验收条件见 [商业级编译器管线方案](COMPILER_PIPELINE.md)。
...
- `BinaryOperator` 是普通与 Matlab 逐元素二元运算的规范身份，`UnaryOperator` 独立保存共轭/非共轭转置；两者随语言 AST、HIR、MIR `OperationAttributeTable` 和两个目标 LIR 传递，源 spelling 只用于诊断/调试。Matlab compatible-size 运算另由逐轴 `BroadcastPlan` 保存两侧规范 shape、结果 shape 和 match/expand mode，逻辑索引使用强类型 `IndexSelection`；HIR/MIR/目标 verifier 逐层拒绝缺失、错误 arity、矛盾 shape 或错误 expression kind。Analyzer、MIR constant folding 和 target representation 不从 renderer 中反向猜测操作。两个目标分别实现二维矩阵乘法、静态 N 维 broadcast、数组比较、vector/rank-2 转置、静态 `end` 和单 mask 线性逻辑读写；动态 extent 与未实现矩阵/索引语义在 target lowering 前失败关闭；
