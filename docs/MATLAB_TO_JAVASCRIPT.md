...
| 表达式 | 标量与数组算术/关系比较、调用、向量/矩阵字面量、转置、上下文 `end`、colon/index/section、线性逻辑索引 | 矩阵除法/幂、complex 转置、短路/逐元素逻辑完整语义、动态 `end` |
| 数据模型 | number、boolean、字符文本的当前子集、矩形嵌套数组 | numeric class、complex、sparse、string、cell、struct、table、datetime、对象 |
| 数组语义 | 1-based、列主序、静态 N 维 reshape/section/compatible-size 隐式扩展、二维矩阵乘法、线性逻辑读写 | 动态 rank/extent 广播、矩阵除法/幂、删除、growth、重复与逐维逻辑索引、完整值语义和 alias 契约 |
| 函数 | 文件级 local function、前向调用、单/多输出 | `nargin`/`nargout`、`varargin`/`varargout`、function handle、anonymous/nested closure、workspace |
| JavaScript runtime | 内嵌数组、索引、section、reshape、flatten-once 广播、转置、逻辑索引和基础 intrinsic runtime | 有版本的 Matlab runtime 包、NDArray 表示、数值/异常兼容层、依赖与许可证审计 |
...
LIR。动态 rank/extent、complex 共轭、逐维逻辑 selector、重复索引、growth、矩阵除法/幂、
...
- [ ] 实现矩阵除法/幂及完整 numeric class 规则
- [x] 实现静态已知 shape 下逐维相容或一侧 extent 为 1 的隐式扩展，并在 HIR/MIR/目标 LIR 显式保存 broadcast plan
- [x] 实现静态 extent 的 `end` 逐索引位置/线性 numel 语义，以及单 mask 线性逻辑读取和标量/vector 写入
- [ ] 实现动态 `end`/bounds、逐维/重复/空索引和多 selector 逻辑索引
...
- [x] 0.4.8 性能门禁增加 N 维 broadcast、转置、数组比较、逻辑索引和 `end` 的专属编译延迟、吞吐与产物大小预算
...
- [ ] 随后纵切面：矩阵除法/幂、dynamic extent、complex/numeric class、逐维/重复索引与 growth
