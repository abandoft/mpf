...
| 表达式 | 标量与数组算术/关系比较、静态满秩稠密实数方阵/超定/欠定求解、safe-integer 方阵幂、调用、向量/矩阵字面量、转置、上下文 `end`、colon/index/section、保序/重复/空 numeric selector、线性及逐维 logical selector | 秩亏求解的 Matlab 兼容行为、非整数矩阵幂、complex 转置、短路/逐元素逻辑完整语义、动态 `end` |
| 数据模型 | number、boolean、字符文本的当前子集、矩形嵌套数组 | numeric class、complex、sparse、string、cell、struct、table、datetime、对象 |
| 数组语义 | 1-based、列主序、静态 N 维 reshape/section/compatible-size 隐式扩展、二维矩阵乘法、静态满秩实数 square/rectangular solve、方阵 integer power、广义 selector 读写 | 动态 rank/extent 广播、秩亏/一般矩阵分解与幂、删除、growth、完整动态 bounds、值语义和 alias 契约 |
...
logical selector，并在生成 runtime 中验证动态已知的 mask extent。动态 rank/extent、complex 共轭、
动态 `end`、growth、秩亏系统的 Matlab 兼容求解、非整数矩阵幂、
...
- [x] 实现静态已知 shape 下逐维相容或一侧 extent 为 1 的隐式扩展，并在 HIR/MIR/目标 LIR 显式保存 broadcast plan
...
- [ ] 实现动态 `end`/完整 bounds、indexed deletion、自动 growth 和动态 shape 变化
...
- [x] 0.4.8 性能门禁增加 N 维 broadcast、转置、数组比较、逻辑索引和 `end` 的专属编译延迟、吞吐与产物大小预算；0.4.9 增加矩阵 solve/power、逐维 logical 与重复 numeric selector 编译场景
...
- [ ] 随后纵切面：秩亏求解兼容策略、dynamic `end`/broadcast extent、complex/numeric class、growth 与 indexed deletion
