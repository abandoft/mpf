...
| 表达式 | 标量与数组算术/关系比较、静态稠密实数方阵及 rank-aware 矩形基本最小二乘求解、safe-integer 方阵幂、调用、向量/矩阵字面量、转置、静态及运行时 extent 的上下文 `end`、colon/index/section、保序/重复/空 numeric selector、线性及逐维 logical selector | 奇异方阵的精确 Matlab 数值/警告行为、非整数矩阵幂、complex 转置、短路/逐元素逻辑完整语义 |
...
`MatrixRankPolicy` 将矩形 `\`/`/` 固定为 rank-aware CPQR 基本最小二乘解：欠定问题不再错误地产生
最小范数解，数值秩亏返回 pivoted basic solution 并在两目标稳定警告；奇异方阵继续走独立 LU/full-rank
边界。一般 NDArray 表示与跨函数动态 shape 数据流、complex 共轭、非整数矩阵幂、
...
- [x] 以部分主元处理 full-rank 方阵；以 rank-aware 带列主元 Householder QR 统一处理超定/欠定矩形系统，返回 Matlab 风格基本最小二乘解，非预期数值秩亏继续计算并稳定警告
- [ ] 实现奇异方阵的精确 Matlab 数值/警告策略、非整数矩阵幂及完整 numeric class 规则
...
- [x] 0.4.8 性能门禁增加 N 维 broadcast、转置、数组比较、逻辑索引和 `end` 的专属编译延迟、吞吐与产物大小预算；0.4.9 增加矩阵 solve/power、逐维 logical 与重复 numeric selector 编译场景；0.5.0 增加动态 `end` 读写和 runtime-shape broadcast 场景；0.5.1 增加 vector/matrix/N 维扩容与删除场景；0.5.2 增加重复零 extent reshape/broadcast/transpose/section/growth 场景
...
- [x] rank-aware solve 纵切面：Semantic v9/MIR v14/LIR v20 显式 `MatrixRankPolicy`，矩形 CPQR 基本解、秩亏 warning、欠定非 minimum-norm 修正、双目标差分/source map/fuzz/性能与奇异方阵负向门禁
- [ ] 随后纵切面：奇异方阵精确兼容、complex/numeric class 与跨函数动态 NDArray ABI
