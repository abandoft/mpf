...
| 数组语义 | 1-based、列主序、规范 `0×0` empty、静态零 extent reshape/transpose/broadcast/section/growth、静态 N 维 reshape/section，以及静态 shape 或 local-function runtime rank/extent 的 compatible-size 隐式扩展；二维矩阵乘法、静态满秩实数 square/rectangular solve、方阵 integer power、广义 selector 读写、vector/matrix/N 维多轴扩容与单轴删除 | 可跨函数携带不可结构恢复 shape 的统一动态 NDArray/typed-array ABI、秩亏/一般矩阵分解与幂、非 vector matrix 线性删除、多轴删除、完整动态 bounds、值语义和 alias 契约 |
...
JavaScript 使用不可枚举 descriptor，C++17 则在静态可知路径消费 input/result shape。一般 NDArray
表示与跨函数动态 shape 数据流、complex 共轭、任意多轴删除、
秩亏系统的 Matlab 兼容求解、非整数矩阵幂、
...
- [x] 实现 vector/matrix/N 维单轴 indexed deletion 与多轴自动 growth；支持 scalar/range/numeric selector、vector 线性语义、运行时标量 selector 和间隙默认值；非 vector matrix 线性删除、多轴删除及缺失维度的 shape-changing assignment 以 `MPF2050` 失败关闭
...
- [ ] 随后纵切面：秩亏求解兼容策略、complex/numeric class、跨函数动态 NDArray ABI 与多轴删除
