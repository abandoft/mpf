...
| 表达式 | 标量与数组算术/关系比较、静态稠密实数方阵的 diagonal/upper/lower/pivoted-tridiagonal/symmetric-positive-definite/dense 结构感知求解及 rank-aware 矩形基本最小二乘求解、safe-integer 方阵幂、调用、向量/矩阵字面量、转置、静态及运行时 extent 的上下文 `end`、colon/index/section、保序/重复/空 numeric selector、线性及逐维 logical selector | symmetric-indefinite 专用分解、Hermitian、sparse 与 complex solver dispatch，病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂、complex 转置、短路/逐元素逻辑完整语义 |
...
数值逐位一致，当前也尚未复刻 Matlab 对 symmetric-indefinite、Hermitian、sparse 或 complex
矩阵的完整 solver dispatch。
...
- [ ] 实现 symmetric-indefinite 专用分解、Hermitian、sparse 与 complex solver dispatch、病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂及完整 numeric class 规则
...
- [x] 0.5.5 高级实数方阵纵切面：Semantic v12/MIR v17/LIR v23 `classify_real_square`、带主元三对角 LU、Cholesky、对称不定 dense fallback、转置条件估计、左右除及差分/source map/fuzz/性能门禁
- [ ] 随后纵切面：symmetric-indefinite 专用分解、Hermitian、sparse/complex 分派与病态特定解精确对齐、完整 numeric class、跨函数动态 NDArray ABI
