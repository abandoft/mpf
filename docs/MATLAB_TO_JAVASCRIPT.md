...
| 维度 | 当前状态 | 商用阻断项 |
|---|---|---|
| 源码与语句 | logical statement、注释、续行、脚本、local function、函数/脚本裸 `return`、`disp`/`display` 单 character-vector command form、分支、循环和标量 `switch` | 通用 command syntax、`try/catch`、`arguments` block、workspace 声明、包/类、产生式级 recovery |
...
一般 NDArray 表示与跨函数动态 shape 数据流、通用 command form、`try/catch`、`arguments`、
...
- [ ] 完成关系、逐元素逻辑、短路逻辑、colon、通用函数/command 调用和转置/字符向量的上下文语法；`disp`/`display` 单参数 command form 已完成
...
- [ ] P0-A：继续补齐 R2024 通用 command/function form、`try/catch`、`arguments` block，并为未支持语法提供精确恢复与诊断
