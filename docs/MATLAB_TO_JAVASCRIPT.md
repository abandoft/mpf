...
| 数组语义 | 1-based、列主序、静态 N 维 reshape/section，以及静态 shape 或 local-function runtime rank/extent 的 compatible-size 隐式扩展；二维矩阵乘法、静态满秩实数 square/rectangular solve、方阵 integer power、广义 selector 读写、vector/matrix/N 维多轴扩容与单轴删除 | 一般 NDArray/typed-array 表示与动态 shape 数据流、秩亏/一般矩阵分解与幂、非 vector matrix 线性删除、多轴删除、完整动态 bounds、值语义和 alias 契约 |
...
独立 runtime 执行。一般 NDArray 表示与跨函数动态 shape 数据流、complex 共轭、任意多轴删除、
...
- [ ] 建立一般空数组 descriptor、跨函数 shape 变化传播和完整 assignment conformability
...
- [x] 0.4.8 性能门禁增加 N 维 broadcast、转置、数组比较、逻辑索引和 `end` 的专属编译延迟、吞吐与产物大小预算；0.4.9 增加矩阵 solve/power、逐维 logical 与重复 numeric selector 编译场景；0.5.0 增加动态 `end` 读写和 runtime-shape broadcast 场景；0.5.1 增加 vector/matrix/N 维扩容与删除场景
...
- [ ] 随后纵切面：秩亏求解兼容策略、complex/numeric class、一般动态 NDArray/空数组 descriptor 与多轴删除
