...
| 表达式 | 标量与数组算术/关系比较、静态稠密实数方阵的 diagonal/upper/lower/dense 结构感知求解及 rank-aware 矩形基本最小二乘求解、safe-integer 方阵幂、调用、向量/矩阵字面量、转置、静态及运行时 extent 的上下文 `end`、colon/index/section、保序/重复/空 numeric selector、线性及逐维 logical selector | tridiagonal、symmetric/Hermitian、positive-definite、sparse 与 complex solver dispatch，病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂、complex 转置、短路/逐元素逻辑完整语义 |
...
`MatrixStructurePolicy` 选择运行时结构检测：exact-zero diagonal、upper-triangular 和
lower-triangular 分别使用直接对角求解或前向/回代，其余方阵回退到部分主元 LU；右除在转置系统
上复用同一分类。各路径以对应正/转置求解构造 1-范数条件估计；`rcond==0` 与
`0<rcond<eps` 分别产生 singular/nearly-singular warning 后继续返回 IEEE `Inf`/`NaN` 或有限病态解。
两目标不要求病态数值逐位一致，当前也尚未复刻 Matlab 对 tridiagonal、symmetric/Hermitian、
positive-definite、sparse 或 complex 矩阵的完整 solver dispatch。
...
- [ ] 实现 tridiagonal、symmetric/Hermitian、positive-definite、sparse 与 complex solver dispatch、病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂及完整 numeric class 规则
...
- [x] 0.5.4 结构感知方阵纵切面：Semantic v11/MIR v16/LIR v22 `MatrixStructurePolicy`、diagonal/upper/lower/dense 双目标分派、结构对应条件估计、左右除、递归数值字面量 widening、差分/source map/fuzz/性能门禁
- [ ] 随后纵切面：tridiagonal、symmetric/Hermitian、positive-definite、sparse/complex 分派与病态特定解精确对齐、完整 numeric class、跨函数动态 NDArray ABI
