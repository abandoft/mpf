...
| 维度 | 当前状态 | 商用阻断项 |
|---|---|---|
| 源码与语句 | logical statement、注释、续行、脚本、local function、函数/脚本裸 `return`、带参数的 local/builtin command syntax、`ans` value/discard、`disp`/`display` 单 character-vector command form、分支、循环和标量 `switch` | 裸无参数与限定名/package/class command、外部 path 解析、`try/catch`、`arguments` block、workspace 声明、包/类、产生式级 recovery |
...
C++ 无法表示或无法证明兼容的顺序重赋值/控制流结果类型在发射前失败关闭。一般 NDArray 表示与跨函数动态 shape 数据流、
裸无参数/限定名/外部 command 解析、`try/catch`、`arguments`、
cell/struct/string 和其余异常结构仍不在当前可保持边界。因此文档、版本说明和 CLI
必须继续使用“已验证子集”的表述。
...
- [ ] 完成 `try/catch`、`global`、`persistent` 和 `arguments` block 的语法与失败关闭边界；函数/脚本裸 `return` 已完成
...
- [ ] P0-A：继续补齐 R2024 裸无参数及限定名/package/class command、外部 path/project 解析、`try/catch`、`arguments` block，并为未支持语法提供精确恢复与诊断
