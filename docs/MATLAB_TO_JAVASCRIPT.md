...
| 维度 | 当前状态 | 商用阻断项 |
|---|---|---|
| 源码与语句 | logical statement、注释、续行、脚本、local function、函数/脚本裸 `return`、带参数的 local/builtin command syntax、未遮蔽零输入 local function 裸调用、`ans` value/discard、`disp`/`display` 单 character-vector command form、分支、循环、标量 `switch`、`try`/单一 `catch [exception]`、格式化 `error`、`MException`、`throw`/`throwAsCaller`/`rethrow`、`addCause`/`getReport`，以及 R2019b positional input/R2022b output `arguments` block 纵切面 | 限定名/package/class command、外部 path/project 解析、name-value/`Repeating` arguments、workspace 声明、公开 cause/stack/correction 属性与完整格式化表面、包/类、产生式级 recovery |
...
availability 按官方引入版本失败关闭：R2020b 才开放 `mustBeFloat`、vector/scalar-or-empty、text、
...
- [ ] 完成 `global`、`persistent`、name-value/`Repeating`/parameterized/custom `arguments`、动态 rank/class ABI、公开 cause/stack/correction 对象属性、完整格式化合同和跨平台 stack frame；函数/脚本裸 `return` 与 `arguments` 第一纵切面已完成
...
- [x] 以 Semantic v33→MIR v39→双目标 LIR v48 完成可结构恢复数组的非全区段动态 assignment：runtime scalar/numeric/logical/range selector、overwrite-or-grow、线性/多维覆盖与扩容、动态单轴删除、N 维页扩容及 copied/staged 事务式回滚
- [ ] 建立可跨函数传播不可结构恢复零 extent 的统一动态 NDArray ABI，并补齐一般 stride/layout、owner/view、copy-on-write 与 escape 优化
...
- [x] 0.7.2 complex sparse matrix-product 纵切面：`MatrixStoragePolicy::sparse_csc_multiply` 与 `MatrixNumericDomain::complex` 贯穿 Semantic v28、MIR v34 与双目标 LIR v42；三种 CSC/dense storage 组合、real/logical promotion、零 extent、canonical CSC、exact-zero cancellation、显式 domain ABI、目标独立 `sparse_product` fragment、source map、差分、runtime 拒错、fuzz 与第 39 项性能预算完成
...
- [`try`/`catch`](https://www.mathworks.com/help/matlab/ref/try.html)
- [Respond to an Exception](https://www.mathworks.com/help/matlab/matlab_prog/respond-to-an-exception.html)
- [`MException`](https://www.mathworks.com/help/matlab/ref/mexception.html)
- [`rethrow`](https://www.mathworks.com/help/matlab/ref/mexception.rethrow.html)
