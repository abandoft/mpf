...
| 表达式 | 标量与数组算术/关系比较、`~`/`&`/`|` compatible-size N 维逐元素逻辑、scalar `&&`/`||` 与 condition-context `&`/`|` 短路、数组 condition 全元素 truthiness、静态稠密实数方阵的 diagonal/upper/lower/pivoted-tridiagonal/symmetric-positive-definite/dense 结构感知求解及 rank-aware 矩形基本最小二乘求解、safe-integer 方阵幂、调用、向量/矩阵字面量、转置、静态及运行时 extent 的上下文 `end`、colon/index/section、保序/重复/空 numeric selector、线性及逐维 logical selector | Hermitian、sparse 与 complex solver dispatch，病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂和 complex 转置 |
...
| 数组语义 | 1-based、列主序、规范 `0×0` empty、静态零 extent reshape/transpose/broadcast/section/growth、静态 N 维 reshape/section，以及静态 shape 或 local-function runtime rank/extent 的 compatible-size 隐式扩展；二维矩阵乘法、rank-aware rectangular solve、方阵 integer power、广义 selector 读写、vector/matrix/N 维多轴扩容，以及符合 null-assignment 规则的单轴删除 | 可跨函数携带不可结构恢复 shape 的统一动态 NDArray/typed-array ABI、一般矩阵分解与幂、完整动态 bounds、值语义和 alias 契约 |
...
- [x] 实现 `~`/`&`/`|` compatible-size N 维逐元素逻辑、scalar `&&`/`||` 短路、condition 非空且全元素非零 truthiness，以及 condition-context scalar `&`/`|` 短路；语义策略贯穿 Semantic v9/MIR v14/LIR v20，并覆盖双目标、拒错、source map、差分、fuzz 和性能
...
- [x] condition-aware square solve 纵切面：Semantic v9/MIR v14/LIR v20 `MatrixConditionPolicy`、部分主元 LU、迭代 1-范数 `rcond`、singular/nearly-singular warning 后继续、双目标左右除差分、warning 次数/source map/fuzz/性能门禁
- [x] 0.5.4 结构感知方阵纵切面：Semantic v9/MIR v14/LIR v20 `MatrixStructurePolicy`、diagonal/upper/lower/dense 双目标分派、结构对应条件估计、左右除、递归数值字面量 widening、差分/source map/fuzz/性能门禁
- [x] 0.5.5 高级实数方阵纵切面：Semantic v9/MIR v14/LIR v20 `classify_real_square`、带主元三对角 LU、Cholesky、对称不定 dense fallback、转置条件估计、左右除及差分/source map/fuzz/性能门禁
