...
| 标量算术与比较 | 支持；equality/ordering/identity/membership 比较链短路且各操作数最多求值一次；bool/number 数值相等，同类 list/tuple 递归相等，list/tuple 跨种类不相等；`None`/布尔 singleton 与 JavaScript sequence reference identity；string/list/tuple membership | 支持；`^`、`.*`、`./`、`.\`、`.^` 保持 Matlab 身份并由目标计划选择实现 | 支持，含点式比较/逻辑运算符 | 标量算术、`===`/`!==` 与 ordering；loose equality 和对象/array reference comparison 失败关闭 |
...
Matlab 算术在 token、AST/HIR/MIR spelling 与目标 LIR 中区分矩阵运算和逐元素运算。当前已验证二维 numeric 矩阵 `*`、完全相同 shape 或一侧为 scalar 的 `+`、`-`、`.*`、`./`、`.\`、`.^`，JavaScript 与生成 C++ 使用各自目标 runtime plan；shape 不一致和当前未实现的数组 `/`、`\`、`^` 以 `MPF2046` 失败关闭。逐维 extent-1 隐式扩展、矩阵除法/幂、转置、empty/complex/sparse/numeric-class 规则尚未支持。
