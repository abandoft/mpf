...
Matlab 算术在 token、AST/HIR/MIR 与目标 LIR 中区分矩阵运算和逐元素运算。当前已验证二维 numeric 矩阵 `*`，静态稠密实数方阵/超定/欠定矩阵的 `\`/`/` 求解与 safe-integer 方阵 `^`，以及静态 shape 或 local-function runtime rank/extent 下 `+`、`-`、`.*`、`./`、`.\`、`.^` 和关系比较的 compatible-size 隐式扩展；方阵求解采用部分主元 LU，并以迭代 1-范数估计区分正常、近奇异和精确奇异条件，矩形求解采用 rank-aware 带列主元 Householder QR，返回 pivoted 基本最小二乘解而不是错误的最小范数解，数值秩低于 `min(rows, columns)` 时继续计算并稳定警告。负整数幂通过求逆与平方递推实现；方阵在 singular/nearly-singular warning 后继续产生 IEEE 或有限病态结果。缺失的尾随维度按 singleton 处理，vector 与高 rank operand 组合时按 Matlab row vector 规范化；runtime-operands plan 在两目标独立验证 rectangularity 与 extent 兼容性，同时保持标量调用结果。共轭 `'` 与非共轭 `.'` 保留独立身份，当前支持非 complex vector/rank-2 operand；上下文 `end` 在静态 shape 上直接折叠，在动态 extent 上通过强类型 axis/linear extent plan 延迟到目标 runtime 求值。保序/重复/空 numeric selector、线性/逐维 logical selector 的读取和标量/vector 写入遵循列主序；静态可证的 mask mismatch 在 Analyzer 拒绝，动态 extent 在生成 runtime 验证。索引写入另由强类型 mutation contract 区分 overwrite/grow/erase，支持 vector、matrix 与一般 N 维数组的多轴扩容、单轴删除、vector 线性扩容/删除、运行时标量 selector 和默认值填隙；重复删除 selector 只删除一次。Matlab null assignment 只允许 vector 线性删除，或恰好一个非 colon selector 所指定的完整轴 slice 删除；非 vector 线性删除、多个非 colon selector 及缺失维度的 shape-changing assignment 是稳定失败边界，动态函数参数还会在生成 runtime 复核 vector shape。一般 NDArray/typed-array 值语义与跨函数 shape 推导、complex 共轭、tridiagonal、symmetric/Hermitian/positive-definite 与 sparse 方阵分派、非整数矩阵幂和 numeric-class 完整规则尚未支持，并分别以 `MPF2046`—`MPF2050` 或稳定 runtime 错误失败关闭。

实数方阵左除和右除现先按精确零模式识别 diagonal、upper-triangular 与 lower-triangular；分别使用直接对角求解或前向/回代，并对其他方阵回退到部分主元 dense LU。右除在转置系统上执行同一结构分类，三类结构路径与 dense 路径都使用相匹配的倒条件估计并在 singular/nearly-singular 时继续警告。该能力不声明容差结构识别，也不包含 tridiagonal、symmetric/Hermitian、positive-definite、sparse 或 complex 分派。
