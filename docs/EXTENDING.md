...
当前 side-table/LIR schema 为 Semantic v32、MIR v38 和双目标 LIR v47。新增带 section
replacement 的源语言或目标时，必须显式产生并消费 `IndexedReplacementContract`：conformability、
static/runtime shape source、selection/value shape 都是必填事实；目标 planner 必须独立重算并选择
runtime ABI，renderer 不得根据 selector 拼写、源语言或 replacement 容器形态恢复规则。至少覆盖
静态 singleton-axis 等价、scalar expansion、动态线性/多维成功与失败、写入前拒错、source map、
逐层损坏、双目标执行、fuzz 和性能预算。
