...
当前 side-table/LIR schema 为 Semantic v33、MIR v39 和双目标 LIR v48。新增带 section
...
新增源语言异常结构时，frontend manifest 必须显式声明 `exception_handling` capability，语言 AST/HIR 必须保存 protected body、handler、可选 binding 与 handler source line，Analyzer 必须固定异常值类型、binding scope、确定赋值和可访问属性。MIR lowering 必须为每个结构建立唯一 `ExceptionRegion`、最内层 exceptional edge 和类型化 capture instruction；verifier、CFG 优化、alias/effect 与 memory-dependence 必须共同理解该边，不能把 handler 作为普通 alternative 猜测。每个目标后端再独立选择 try/catch statement form、binding access、异常 runtime fragment 与 source mapping；renderer 只序列化 LIR，目标之间不得共享 runtime token 或生成产物。新增异常 form 至少覆盖正常跳过、运行时失败、嵌套优先级、重抛、无 binding handler、确定赋值、区域损坏拒绝、双目标执行、source map、fuzz 和独立性能预算。
