...
storage/shape、compatible-size 逐轴广播以及 preserve-sparse/materialize-dense policy；logical operand
按 Matlab 数值算术提升为 binary64。HIR、MIR 和两个目标 verifier 必须逐层重算完整计划及 inactive
state；JavaScript/`cpp` representation planner 分别绑定目标专属 helper，并把 operation、policy、三项
storage 与完整 shape ABI 固化进 LIR。sparse-sparse 路径必须直接归并 canonical CSC 列并删除
exact-zero cancellation，mixed sparse/dense/scalar 路径必须物化 dense 结果。kernel 必须位于依赖
sparse base 的目标私有可选 runtime fragment；未使用 sparse 加减的产物不得携带它。新增 form 至少覆盖
双向 scalar、row/column/outer expansion、logical 提升、零 extent、nonfinite/overflow、跨层/目标
