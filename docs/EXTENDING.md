...
当前 side-table/LIR schema 为 Semantic v34、MIR v40 和双目标 LIR v49。新增带 section
...
planner 必须独立重算并选择 runtime ABI，renderer 不得根据 selector 拼写、源语言或 replacement
容器形态恢复规则。runtime 必须先验证 selector、input/selection/replacement/result shape，再通过
copied/staged root 原子提交。至少覆盖静态 singleton-axis 等价、scalar expansion、动态线性/多维
覆盖与增长、logical/numeric/range selector、删除、失败回滚、source map、逐层损坏、双目标执行、
fuzz 和性能预算。
...
新增源语言异常结构时，frontend manifest 必须显式声明 `exception_handling` capability，语言 AST/HIR 必须保存 protected body、handler、可选 binding 与 handler source line，Analyzer 必须固定异常值类型、binding scope、确定赋值和可访问属性。构造、抛出、cause/report 等异常表达式还必须提供强类型 operation、message form、stack policy 与结果类型，不能用 intrinsic/helper 拼写作为隐式合同。MIR lowering 必须为每个结构建立唯一 `ExceptionRegion`、最内层 exceptional edge 和类型化 capture instruction，并在稠密 expression attribute 中传播异常计划；verifier、CFG 优化、alias/effect 与 memory-dependence 必须共同理解该边和 may-fail/control effect，不能把 handler 作为普通 alternative 猜测。每个目标后端再独立重算异常计划，选择 try/catch statement form、binding access、异常 runtime fragment 与 source mapping；renderer 只序列化 LIR，目标之间不得共享 runtime token 或生成产物。新增异常 form 至少覆盖正常跳过、运行时失败、嵌套优先级、stack policy、重抛、无 binding handler、确定赋值、区域/计划损坏拒绝、双目标执行、source map、fuzz 和独立性能预算。
...
必要字段。当前 static real/logical rank-2 CSC `all`/`any` 证明了这一接口：目标 planner 固化四组
shape 和四个 integer argument，非标量结果保持 logical CSC，全维结果转为 full logical scalar；
目标 runtime catalog 将归约 kernel 作为依赖 sparse base 的可选 fragment，而不是塞入所有 CSC 程序。

新增 matrix-power storage form 时，前端必须先用 `MatrixExponentPolicy` 固定指数域，再由
`MatrixOperationPlan` 固定 base/result storage 与 shape。当前 sparse contract 只接受静态 real/logical
square CSC 和 nonnegative ECMAScript-safe integer；logical base 的结果提升为 canonical double CSC。
两个目标必须各自实现 CSC×CSC 重复平方法、零次幂 identity、动态指数检查和 overflow 拒绝，并将
power kernel 作为依赖 sparse base 的独立可选 fragment。目标 verifier 必须重算两组 shape、三项
integer ABI 与 fragment closure；renderer 不得从 `^`、helper 拼写或 operand 表示重新决定指数或
storage policy。
...
新增 sparse constructor form 时，frontend 必须先扩展 `SparseConstructionKind` 与 `SparseConstructionPlan`，由 HIR/MIR verifier 重算 arity、shape、triplet cardinality 和 inactive state，再分别在 JavaScript/`cpp` LIR 中验证并选择目标 form；renderer 不得读取该计划、按参数数量猜 helper 或复用另一后端 runtime。

新增 sparse stored-value domain 时还必须扩展 `SparseValueDomain`、duplicate policy、element `NumericType`
和 canonical CSC validator，并让 construction、index、mutation、reshape、transpose、`full` 与 `nnz`
继承同一数值合同。跨 feature 的运行时代码必须形成显式 target LIR fragment：当前
`complex_sparse` 必须由实际 complex CSC 值显式触发并验证对 complex scalar、array 与 sparse base
feature 的依赖闭包；无关 complex scalar 与 real CSC 同时存在时不得进入 module/translation-unit plan。
JavaScript 与 `cpp` 各自序列化本目标 fragment，基础 sparse runtime 不拥有 complex 专属入口。
...
新增 JavaScript 可变容器表示时必须声明源语言 ownership。Matlab dense array 的 name-to-name assignment 和会被索引修改的 local-function value parameter 分别进入 `AssignmentValueForm::copy_array` 与 `ParameterPassing::value_copy`；Matlab sparse mutation 必须进入 `MutationOwnership::replace_with_result` 并返回新根值。不得对 fresh target 的每次写入盲目深拷贝，也不得让 Matlab alias 退化为 JavaScript reference alias；Python/TypeScript reference、Fortran writable reference 和 C++ value container 不能被统一成同一隐式策略。新增表示至少覆盖直接 alias、函数参数、growth/erase、dense/sparse、双目标差分、LIR 计划损坏和性能预算。
...
新增源语言归约 intrinsic 时，Analyzer 必须产出独立 `ReductionPlan`，明确 operation、axis policy、shape source、输入/输出 shape、归约 axis、scalar identity、`ReductionStoragePolicy` 与输入/结果 storage。Semantic/MIR/目标 verifier 应分别从 intrinsic 与 operand facts 重算契约；优化删除表达式时必须清空 plan。目标 representation 只选择已绑定的 static reduction 或 runtime total helper，并把完整 shape/integer ABI 固化到目标 LIR；renderer 不得解析 `dim`、扫描 shape、读取 storage policy 或根据 helper 拼写决定 `all`/`any`。新增 sparse form 时还必须把 kernel 放入依赖 sparse base 的独立可选 runtime fragment，并验证不使用该 form 的生成产物不携带它。新增动态维度、字符数组或未知 rank 保持维度的归约前，应先扩展统一 NDArray/numeric-class ABI，再补双目标 runtime、空维 identity、损坏 plan、差分、source map、fuzz 和性能门禁。
...
新增 sparse 逐元素运算时，不得复用矩阵 `*` 的 `MatrixOperationPlan`。Analyzer 必须生成独立
`SparseElementwisePlan`，显式固定 operation、preserve-sparse policy、左右 operand 的 storage/shape、
每个 compatible-size broadcast axis 和结果 shape；HIR、MIR 与两个目标 verifier 必须逐层重算完整
计划，并在节点失活时拒绝残留事实。JavaScript/`cpp` representation planner 分别选择 CSC×scalar、
scalar×CSC、CSC×dense、dense×CSC 或 CSC×CSC helper，renderer 只能序列化已绑定参数和 shape。
runtime 必须验证 canonical CSC、finite input/result 与计划 shape，按非零项驱动或合并 CSC 列完成
singleton row/column expansion，删除 exact-zero result 并直接返回 canonical CSC。新增 operation 至少
覆盖五种 storage/operand 方向、双轴广播、损坏计划、source map、双目标差分、生成 runtime 拒错、
fuzz、架构隔离和独立性能预算。

新增 sparse 数值加减时，不得复用 `SparseElementwisePlan` 或目标 runtime tag 推断存储行为。Analyzer
必须生成独立 `SparseArithmeticPlan`，显式固定 `add`/`subtract` identity、左右 operand 的
storage/shape、finite-real/finite-complex value domain、compatible-size 逐轴广播以及 preserve-sparse/materialize-dense policy；logical operand
按 Matlab 数值算术提升为 binary64，与 complex operand 组合时进一步提升为 complex。HIR、MIR 和两个目标 verifier 必须逐层重算完整计划及 inactive
state；JavaScript/`cpp` representation planner 分别绑定目标专属 helper，并把 operation、policy、三项
storage、value domain 与完整 shape ABI 固化进 LIR。sparse-sparse 路径必须直接归并 canonical double/complex CSC 列并删除
exact-zero cancellation，mixed sparse/dense/scalar 路径必须物化对应 numeric domain 的 dense 结果。kernel 必须位于依赖
sparse base 的目标私有可选 runtime fragment；未使用 sparse 加减的产物不得携带它。新增 form 至少覆盖
双向 scalar、row/column/outer expansion、logical/complex 提升、零 extent、nonfinite/overflow、跨层/目标
LIR/生成 ABI 损坏、source map、双目标差分、严格生成 C++ 编译、fuzz、纯 Emitter 架构隔离和独立性能预算。

新增 sparse logical 运算时，不得复用 numeric `SparseElementwisePlan` 或让目标根据 operand runtime tag
临时决定结果存储。Analyzer 必须生成独立 `SparseLogicalPlan`，显式固定 `logical_not`、
`logical_and` 或 `logical_or` identity、左右 operand 的 storage/shape、compatible-size 结果 shape，
以及 `preserve_sparse` 或 `materialize_dense` policy。HIR、MIR 与两个目标 verifier 必须逐层重算
arity、static rank-2 shape、storage policy 和 inactive state；JavaScript/`cpp` representation planner
分别绑定目标专属 helper，并把 operation、policy 与三项 storage 枚举固化到
`runtime_integer_arguments`。renderer 只能序列化已验证 ABI。runtime 必须独立验证 canonical CSC 与
shape plan：`~S`、至少一侧 sparse 的 `&`、sparse-sparse `|` 直接产生 canonical logical CSC，mixed
sparse-dense/scalar `|` 物化 logical dense 结果；不得借助另一目标产物或先把所有 sparse operand
无条件转为 dense。新增 form 至少覆盖零 extent、row/column broadcast、storage-preserving 与
materializing 分支、跨层/目标 LIR/生成 ABI 损坏、source map、双目标差分、严格生成 C++ 编译、fuzz、
纯 Emitter 架构隔离和独立性能预算。
