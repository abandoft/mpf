...
- `0.0.9` 的下一个版本是 `0.1.0`；
- `0.2.9` 的下一个版本是 `0.3.0`；
- 当前发布版本是 `0.3.5`；下一开发版本按路线图从 `0.3.6` 开始。
...
Git tag 使用 `vMAJOR.MINOR.PATCH`。正式发布时必须把对应条目从 `Unreleased` 固化为版本标题，
并保证 tag、CMake、CLI、安装包和 changelog 版本完全一致。Release workflow 会在打包前拒绝与
CMake 项目版本不一致的 tag。
...
会拒绝少于 8 条或多于 20 条的版本段，并要求文件顶部继续保留新的 `Unreleased` 段。
