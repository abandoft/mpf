...
Git tag 使用 `vMAJOR.MINOR.PATCH`。`CHANGELOG-ZH.md` 不设置待发布占位段，必须直接以当前 CMake 项目版本标题开头。changelog 保存开发快照的工程变化，但任何历史条目都不构成跨版本兼容承诺。发布检查必须保证 tag、CMake、CLI、安装包、性能基线和 changelog 版本完全一致。
