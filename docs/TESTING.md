...
|---|---:|
| C++ 单元与集成测试 | 216 项，零失败 |
| CTest | Debug/Release/RelWithDebInfo 均为 93 项；包含 79 项 differential、1 项 C++ 单元/集成、3 项生成 runtime 拒绝、1 项 fuzz smoke、1 项编译器分层门禁、1 项发布脚本正/负契约、2 项 CLI、1 项生成 C++ 编译、3 项后端隔离和 1 项安装后示例测试；非插桩性能发布门禁由独立目标执行，不重复计入普通 CTest |
| Differential corpus | Python 22、Fortran 19、Matlab 34、TypeScript 4，共 79 个 case |
| 工具完整环境执行路径 | 203 条程序路径，另有每 case 一条 oracle |
...
- 34 个 Matlab case：Node.js、生成 C++17 与 oracle 三路比较；
