...
|---|---:|
| C++ 单元与集成测试 | 295 项，零失败 |
| CTest | Debug/Release/RelWithDebInfo 均为 147 项普通测试；包含 104 项 differential、1 项 C++ 单元/集成、32 项生成 runtime 拒绝、1 项 fuzz smoke、1 项编译器分层门禁、1 项发布脚本正/负契约、2 项 CLI、1 项生成 C++ 编译、3 项后端隔离和 1 项安装后示例测试；Release 流程另运行不计入 CTest 的独立性能发布目标 |
| Differential corpus | Python 22、Fortran 19、Matlab 59、TypeScript 4，共 104 个 case |
| 工具完整环境执行路径 | 253 条程序路径，另有每 case 一条 oracle |
...
- 59 个 Matlab case：Node.js、生成 C++17 与 oracle 三路比较；
