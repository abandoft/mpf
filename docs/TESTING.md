...
|---|---:|
| C++ 单元与集成测试 | 237 项，零失败 |
| CTest | Debug/Release/RelWithDebInfo 均为 109 项；包含 91 项 differential、1 项 C++ 单元/集成、7 项生成 runtime 拒绝、1 项 fuzz smoke、1 项编译器分层门禁、1 项发布脚本正/负契约、2 项 CLI、1 项生成 C++ 编译、3 项后端隔离和 1 项安装后示例测试；非插桩性能发布门禁由独立目标执行，不重复计入普通 CTest |
| Differential corpus | Python 22、Fortran 19、Matlab 46、TypeScript 4，共 91 个 case |
| 工具完整环境执行路径 | 227 条程序路径，另有每 case 一条 oracle |
| 生产代码行覆盖率 | 0.6.1 实测 91.10%（32,991/36,213），硬门槛 85% |
...
- 46 个 Matlab case：Node.js、生成 C++17 与 oracle 三路比较；
