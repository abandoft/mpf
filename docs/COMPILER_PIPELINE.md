...
- 显式 exception contract；AST/HIR 保存 protected body、单一 handler、可选 binding 和 handler source line，Analyzer 固定 catch value 类型与确定赋值，MIR 以 exception region、exceptional edge 和 capture instruction 表示控制/存储，目标 LIR 只选择本目标的 try/catch form 与 runtime fragment；renderer 不得扫描 source statement 或借用另一目标的异常表示；
- 可扩展的 effect 与可能失败操作。

### 类型、shape 与 storage

- `TypeId` 描述标量、tuple、sequence、array、function/reference 等逻辑类型。
- `NumericType` 是 `TypeId` 内与 coarse `ValueType` 正交的数值 contract：`NumericClass` 当前包含 logical、signed-integer、binary64，`NumericComplexity` 包含 real、complex；`unknown` 表示必须延迟到运行时的参数相关表示，不能被 type interner 当成 real 默认值。expression/element/tuple/statement/parameter/return/multi-target 都必须有稠密 side-table 行，Semantic v33、MIR v39 和双目标 LIR v48 分别验证 class/complexity 组合与结构类型的一致性。
