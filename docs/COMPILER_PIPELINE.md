...
CSC identity，logical base 在结果中提升为 double。`sparse_power` 是依赖 `sparse_matrices` 的独立按需
fragment，未使用该能力的 translation unit 不得携带其代码；renderer 只序列化已验证的 helper 与
shape/integer 实参。
