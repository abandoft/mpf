...
- 在 program semantic profile 与 side table 中描述 truthiness、`LogicalEvaluation`、division、index、layout、comparison 等规则，不靠后端按 `SourceLanguage` 或 operator spelling 猜测；
...
- 显式 logical evaluation policy；eager element-wise、boolean short-circuit 与 operand-returning short-circuit 必须在 CFG lowering 前确定，目标 LIR 只能选择表示形式；
- 显式 reduction contract；`all`/`any` 的 operation、默认/显式/全维 axis policy、shape source、输入/输出 shape 与空归约 identity 必须由 Analyzer 固定，目标不得从 helper 名称或嵌套容器恢复；
- 显式 scalar-division contract；quotient 表示与除零行为必须分别由 `Division`/`DivisionByZero` 固定，目标 LIR 只能选择已验证的原生运算或 target-owned runtime call，renderer 不得从 `/`、source language 或 child literal 推断；
- 显式 Matlab command-result contract；frontend scanner 负责 command/expression 空格消歧与字符向量参数边界，AST/HIR 只携带 `ImplicitResultPolicy`，Analyzer side table 决定产值、`ans` 的 `SymbolId` 和 `previous_assigned`，MIR 选择 store 或纯 expression，目标 LIR 选择 value/discard statement form；C++ capability 另以声明 type probe 验证顺序重赋值和首次赋值控制流路径，renderer 不得从 callee 名、返回类型或源码 spelling 推导 `ans`；
- 显式 exception contract；AST/HIR 保存 protected body、单一 handler、可选 binding 和 handler source line，Analyzer 固定 catch value 类型、确定赋值，以及 `MException`/`error`/`throw`/`throwAsCaller`/`rethrow`/`addCause`/`getReport` 的 operation、message form 与 stack policy；MIR 以 exception region、exceptional edge、capture instruction 和稠密 expression attribute 表示控制/存储/失败效果，两个目标 LIR 独立重算结果类型与策略并选择本目标 try/catch form、expression plan 和按需 runtime fragment；renderer 不得扫描 source statement、从 helper 名称恢复异常策略或借用另一目标的异常表示；
- 可扩展的 effect 与可能失败操作。

### 类型、shape 与 storage

- `TypeId` 描述标量、tuple、sequence、array、function/reference 等逻辑类型。
- `NumericType` 是 `TypeId` 内与 coarse `ValueType` 正交的数值 contract：`NumericClass` 当前包含 logical、signed-integer、binary64，`NumericComplexity` 包含 real、complex；`unknown` 表示必须延迟到运行时的参数相关表示，不能被 type interner 当成 real 默认值。expression/element/tuple/statement/parameter/return/multi-target 都必须有稠密 side-table 行，Semantic v35、MIR v41 和双目标 LIR v51 分别验证 class/complexity 组合与结构类型的一致性。
- `ArrayStorageFormat` 与数值类别、complexity 和 shape 正交，当前显式区分 `none`、`unknown`、`dense` 与 canonical `sparse_csc`。expression/statement/tuple element/parameter/return/multi-target 必须具有稠密 storage side-table 行；控制流 join 只在两侧 storage 相同或一侧为未知时保留确定事实，不能把 dense/sparse 分歧猜成任一具体表示。
...
- MIR v41 不把区域或 effect 塞回 `Instruction`；revision-bound `OperationAttributeTable` 按 `InstructionId` 稠密保存 `InstructionAttributes`，每条指令可持有零到多个 `MemoryAccess {storage, root, region, mode}`。expression 属性以强类型 `BroadcastPlan` 区分 Matlab `static_extents`/`runtime_operands` shape source 并保存可用的逐轴 mode，以 `ReductionPlan` 保存 logical operation、axis policy、static/runtime shape source、归约轴和输入/输出 shape ID，以 `SparseConstructionPlan` 保存 constructor kind、value domain、duplicate policy、result shape ID、triplet cardinality 与 reserve hint，以 `SparseIndexPlan` 保存 linear/subscript scalar 或 storage-preserving selection identity、输入/结果 shape ID 与 storage，以 `SparseMutationPlan` 保存 linear/subscript assignment、linear/axis deletion、四类 shape ID、dense/CSC replacement storage、scalar expansion、last-write-wins 与 zero erase policy，以 `SparseReshapePlan` 保存 size-vector/dimension-list identity、单个推断轴、source/result storage 和 input/requested/result shape ID，以 `SparseElementwisePlan` 保存 `.*` operation、preserve-sparse policy、左右 storage/shape、逐轴 compatible-size broadcast mode 和结果 shape，以 `SparseArithmeticPlan` 保存 `+`/`-` operation、preserve-sparse/materialize-dense policy、左右 storage/shape、逐轴 compatible-size broadcast mode 和结果 shape，以 `SparseLogicalPlan` 保存 `~`/`&`/`|` operation、preserve-sparse/materialize-dense policy、左右 storage/shape、逐轴 compatible-size broadcast mode 和结果 shape，以 `MatrixOperationPlan` 保存 matrix kind、real/complex numeric domain、dense/CSC coefficient/product/scale/power storage policy、signed/nonnegative safe-integer exponent policy、square/overdetermined/underdetermined solve kind、condition/factorization/structure policy 和输入/输出 shape ID，以逐下标 `IndexSelectorKind` 保存 scalar/slice/numeric/logical/empty/runtime identity；runtime selector 仅属于无法静态确定类别的 Matlab 索引赋值目标，并以 `IndexExtentSource` 保存静态、运行时 axis 或运行时 linear extent 来源。Matlab empty literal 则由 Semantic v35 固定为 real、dense、column-major、`0×0` sequence，目标 LIR v51 再用 `ArrayLiteralPlan` 区分 direct 与 shaped-empty 表示。直接 load/index/slice 产生 read，store/store-indexed/loop-variable/writeback 产生 write，copy-in/out 只在 copy-in 阶段读取 original region，copy-out 不伪造读取；调用自身的跨函数读写在 alias/effect fixed point 中按 actual region 实例化。
- `IndexedReplacementContract` 与 `IndexedMutationContract` 正交；前者只决定 Matlab replacement 的 scalar expansion、linear element count、nonsingleton shape 或 runtime dispatch，并保存 static/runtime shape source 与 selection/value `ShapeId`；后者以 `overwrite_or_grow` 表示必须由实际 selector 和输入 shape 决定覆盖或扩容的写入。Semantic v35、MIR v41 和双目标 LIR v51 必须从 selector/type/shape 独立重算，full-colon 未知形状写入必须保持目标 shape；目标 runtime 在任何 store 或 resize 前完成实际 selector、结果 shape 和 RHS 相容性验证，并通过 copied/staged root 原子提交增长，renderer 只序列化合同枚举。
- `ArgumentValidationPlan` 是 Analyzer side table 中的 formal contract，保存 ordinal、input/output direction、declared dimensions、class、validator、default 和 validated representation rank；`ArgumentCallBoundary` 是 MIR call-site contract，以独立 flag 保存 class/size conversion，并绑定 validated type/shape/rank。MIR verifier 必须从 callee formal 独立重算实际 boundary，目标 LIR verifier 再检查每个目标 call-argument ABI；renderer 不得读取 `ValueType`、parameter shape 或源 declaration 重新决定 rank/conversion。默认值必须在前序 formal 完成 conversion/validation 后按声明顺序求值，输出在所有返回边界验证。动态 rank、name-value、Repeating 和 parameterized/custom validator 必须先扩展这些强类型 contract，不能只增加 parser 分支或 runtime helper。
- `ExceptionRegion` 独立保存 try owner、protected entry/block inventory、handler、normal continuation 和可选 exception `SymbolId`；每个 protected block 的 `exception_handler` 只指向最内层 handler，catch binding 以 `catch_exception` resident instruction 写入专属 storage。区域、block edge、capture type/storage 和函数所有权由 MIR verifier 双向核对；CFG 优化必须保留或原子重映射这些结构，alias/effect 将 capture 视为 write/control，memory-dependence fixed point 将 exceptional edge 纳入前驱/后继图。
- `MatrixOperationPlan` 以 `MatrixNumericDomain` 固定 real/complex kernel family，以 `MatrixStoragePolicy` 固定 dense、sparse-CSC coefficient、sparse-CSC product、sparse-CSC scale 或 sparse-CSC power contract，以 `MatrixExponentPolicy` 固定 signed 或 nonnegative ECMAScript-safe-integer exponent contract，以 `MatrixFactorizationPolicy` 固定 algorithm family，并以 `MatrixStructurePolicy` 固定允许的结构检测：dense real square solve 必须携带 `classify_real_square`，dense complex square solve 必须携带 `classify_complex_square`，real/complex rectangular solve 必须携带 `rank_revealing_column_pivoted_qr`，sparse real square solve 必须携带 `sparse_csc_coefficient`、`sparse_row_pivoted_lu` 和 `classify_sparse_real_square`；静态 finite-real rank-2 乘法只要一侧为 CSC 就必须携带 `sparse_csc_multiply`，并按 sparse×sparse→CSC、混合乘法→dense 固定结果 storage；一侧为 CSC、另一侧为 finite real/logical scalar 时必须携带 `sparse_csc_scale`，双向次序都固定为 canonical CSC 结果；静态 real/logical square CSC 的 `^` 必须携带 `sparse_csc_power` 和 `nonnegative_safe_integer`，并固定 canonical double CSC 结果；dense multiply 使用 dense/none，dense power 使用 dense/signed-safe-integer 合同。SemanticTable、MIR verifier 和两个目标 LIR verifier 都从 typed operands、storage、operation、exponent policy 和 solve kind 独立重算这些关系；retired/inactive 节点必须清空 domain、storage、exponent、policy 和 shape，避免 target helper 名称成为隐式语义通道。新增 storage 或 algorithm 分派必须先增加 typed contract，不能借用已有枚举静默分派。
- `ReductionPlan` 以 `ReductionStoragePolicy` 区分 dense、preserve-sparse 与 scalar-full 结果，并显式保存输入/结果 storage。静态 real/logical rank-2 CSC 的维度保持型 `all`/`any` 必须产生 canonical logical CSC，全维归约必须产生 full logical scalar；shape、axis、empty identity 和 storage 关系由 SemanticTable、MIR 与两个目标 LIR verifier 分别重算，不能由 runtime tag 或 helper 名称决定。
- `SparseArithmeticPlan` 以 `SparseValueDomain` 固定 finite-real 或 finite-complex 结果域，并与 operation、preserve-sparse/materialize-dense policy、两侧/结果 storage、static compatible-size shape 和逐轴 broadcast mode 一起逐层验证。empty CSC 不能通过存储内容恢复 numeric class；目标 ABI 因此必须显式序列化 value-domain integer。complex 与 real/logical operand 组合执行 complex promotion，sparse-sparse 产生 canonical double/complex CSC，mixed dense/scalar 产生对应 numeric domain 的 dense result；inactive 或 retired plan 必须清空 value domain。
- JavaScript LIR 必须显式规划 Matlab value ownership：dense array identifier assignment 使用 `AssignmentValueForm::copy_array`，可能被索引修改或静态已知为 dense array 的 local-function 形参使用 `ParameterPassing::value_copy`，两者都只在共享边界复制一次；sparse indexed mutation 使用 `MutationOwnership::replace_with_result` 并由 immutable CSC runtime 返回新根值。renderer 只能序列化这些计划，不能从 source language、变量名或 helper 拼写临时决定复制；C++ 值容器、Python/TypeScript 引用和 Fortran writable-reference ABI 保持各自独立策略。
...
`IndexedMutationContract` 是索引写入唯一的 shape 变化事实，区分 overwrite/resize/grow/erase/overwrite-or-grow，并携带 static/runtime selector shape source、线性列主序身份、删除轴和输入/结果 `ShapeId`。Analyzer 生成 contract；MIR 和两个目标 LIR 只转换并验证它。growth/deletion/overwrite-or-grow 的 memory access 必须覆盖整个 aggregate root，禁止沿用 mutation 前的局部 section region；两个 emitter 不得从空 RHS 或 selector 语法重新推断删除/扩容。动态目标先完成 selector、selection、RHS 与 result shape 验证，再通过 JavaScript copied root 或 C++ staged value 提交，失败路径不得留下部分增长或部分写入。目标 lowering 仅在 LIR 实际含 runtime selector 或 dense Matlab indexed replacement 时分别声明对应 runtime feature，fragment verifier 强制其依赖 arrays；静态 sparse mutation 不装载这两类辅助代码。
...
- 函数级 write summary 可裁剪不可能与该函数任一写访问冲突的纯 read frontier；证明同时使用 alias relation 与已建立 element/section view 的 aggregate root 身份，未知类型、未知内存或可能 alias 时仍保守保留，不能以该优化删除任何可形成 WAR 的 read；
- 已验证的 full-root write 先与当前 frontier 形成全部必要 RAW/WAR/WAW，再裁剪被覆盖的同根已知访问；unknown site、不同根和非完整区域不得使用此 kill，长序列 shape mutation 的 frontier 因而保持有界而不损失依赖；
...
Matlab 裸 `return` 的 source-level 输出身份先由 NameTable 绑定为所在函数按声明顺序排列的结果 `SymbolId`，HIR semantic verifier 与 MIR verifier 再逐层重算。目标 representation 必须在已知函数上下文中选择 `return_outputs`，在 Matlab script 中选择 `return_program`；JavaScript `ModulePlan` 进一步显式保存 control prelude、受控 executable body、模块级 local-function declaration 顺序和碰撞安全 label，C++ 则把 script return 绑定为生成入口退出。renderer 只序列化该计划，不能扫描语句种类决定模块包裹边界或重新解析返回名称。
...
当前 LIR v51 已把函数 calling convention、source export、临时值、顶层/函数/statement/body/alternative 作用域声明、顶层布局、表达式/语句、numeric class/complexity、array storage、当前 writable-call ownership/copy-in-out、Matlab argument formal validation/call-boundary conversion、强类型 comparison、logical-evaluation/truthiness、scalar-division/zero-denominator policy、Matlab array-literal/broadcast/reduction shape source 与 reduction storage policy、indexed-replacement conformability/shape source/selection/value shape、runtime selector 与 overwrite-or-grow mutation、sparse/matrix operation plan、逐下标 selector/runtime-extent identity、目标 runtime 调用 ABI、try/catch/exception plan 和逐节点 source mapping representation 前移。expression、statement、parameter、return、multi-target 与 C++ declaration 保存 `SymbolId` 和对应 numeric/storage contract，`IdentifierInventory` 验证 identity/spelling 后生成目标保留字安全名称。每个 call argument 使用一个聚合 plan 保存 passing/writeback 与 `ArgumentCallBoundary`，避免并行数组；C++ statement plan 另保存每条 argument default 的 direct/class/size materialization form，并由 representation verifier 独立重算。renderer 只消费已验证的 target form/token/shape/rank/boundary/default form。JavaScript LIR 绑定 object runtime，`cpp` LIR 绑定 `std::complex<double>` runtime，未使用的 argument-validation、complex 或 matrix fragment 均按依赖裁剪。

Matlab runtime-operands broadcast 由目标独立推导矩形 shape、规范化 row vector/trailing singleton 并验证兼容性；logical reduction 直接消费 operation/axis/shape plan，静态维度保持输出 rank，未知 rank 只允许全维 total reduction；shaped-empty array literal、静态 section result、transpose、length 与 growth 直接消费目标 plan 的 shape，不从嵌套容器反推不可见的零 extent；动态 `end` 由 `IndexExtentSource` 明确选择当前轴长度或列主序 numel。矩形 real/complex solve 只能选择 rank-aware basic-solution 与 CPQR factorization policy，双目标 runtime 独立执行分解、秩判断和 warning；dense real/complex 方阵选择 square-continue-with-warning policy，并分别消费 real transpose 或 complex conjugate-transpose 结构求解与 1-范数条件估计。sparse mutation 由双目标各自执行 CSC update sort/collapse/merge，全部验证后才提交且不物化 sparse source；sparse CSC 实数方阵选择 sparse-row-pivoted-LU contract，目标 runtime 独立验证 canonical CSC，并按三对角 O(nnz) 快路或一般稀疏行主元 LU 求解而不物化稠密系数副本，`0×0` 系数按三组 shape ABI 直接产生 dense/CSC shaped-empty 结果；sparse product 由 JavaScript/`cpp` 各自把同一 storage 与 numeric-domain plan 绑定到三种 matrix-product target helper，real/logical/complex 输入按明确 domain 提升，CSC×CSC 走按列 sparse accumulator并产生 canonical CSC，混合乘法只遍历 CSC 非零项且直接产生 dense 结果；该 kernel 位于独立 `sparse_product` fragment，只有实际 product 或显式依赖它的 sparse power 才会装载；scalar product 另按操作数方向绑定两个 scale helper，非零因子以 O(nnz) 生成 canonical CSC，零因子以 O(columns) 生成同 shape 空 CSC；sparse element-wise product 独立绑定 CSC×scalar、scalar×CSC、CSC×dense、dense×CSC 或 CSC×CSC helper；sparse arithmetic plan 独立绑定 `+`/`-` helper，sparse-sparse 直接归并 canonical CSC，mixed sparse/dense/scalar 物化 dense；sparse logical plan 独立绑定 NOT/AND/OR helper，并按 storage policy 在 CSC 保持与 mixed OR dense materialization 间选择。三类 sparse plan 都执行 compatible-size expansion，renderer 不从 helper 名称恢复 operation 或 storage。

稀疏 logical reduction 继续使用统一 `ReductionPlan`，但目标 representation 必须选择自己的
sparse-reduction helper，并把 input/axes/result/output 四组 shape 与 operation/policy/input-storage/
result-storage 四个整数按固定顺序写入目标调用 ABI。JavaScript 与 `cpp` runtime 都以 canonical CSC
非零项驱动列归约、行归约、全维归约和高于 rank 的 logicalization；复杂度为 O(nnz +
output-extent)，不允许为复用 dense kernel 先物化输入。runtime requirement 将其规划成依赖 CSC base
的独立 `sparse_reductions` fragment，只有实际用到稀疏 `all`/`any` 的 translation unit 才发射该代码；
module/translation-unit verifier 同时检查 fragment 顺序和依赖闭包。

稀疏方阵幂继续使用统一 `MatrixOperationPlan`，但只允许 static real/logical square CSC base 与
nonnegative ECMAScript-safe-integer exponent。两个 representation planner 分别绑定目标专属
sparse-power helper，并把 base/result 两组 shape 与 exponent/input-storage/result-storage 三个整数
按固定顺序写入目标调用 ABI；动态指数由 runtime 再验证，静态非法指数由 Analyzer 先拒绝。JavaScript
与 `cpp` runtime 都以自己的 CSC×CSC kernel 执行重复平方法，零次幂直接产生同阶 canonical double
CSC identity，logical base 在结果中提升为 double。`sparse_power` 是依赖 `sparse_product` 的独立按需
fragment，后者再依赖 `sparse_matrices`；未使用该能力的 translation unit 不得携带其代码，renderer 只序列化已验证的 helper 与
shape/integer 实参。

C++ 类型探针保持严格 C++17，`cpp` 二元 comparison 同样拥有两个稠密 operand temporary，保证左到右单次求值。`SourceSegmentPlan` 按 `LirNodeId` 稠密保存 source/origin，renderer 只能用 ID 查询并产生 marker。独立 representation verifier 在 ABI/resource planner 后重建 expression、statement、scope、numeric/storage contract、comparison、division/runtime call、array literal、broadcast/reduction/sparse construction/sparse index/sparse mutation/sparse reshape/sparse elementwise/sparse arithmetic/sparse logical/matrix operation、runtime shape/integer arguments、selector、extent、evaluation、writeback 和 source segment 计划；deterministic dump 公开 shape/integer ABI。架构门禁禁止 renderer 恢复共享语义、按 spelling 查 binding、扫描 call argument、读取 semantic matrix/storage policy、读取节点 location/line/origin 或自行选择 wrapper。完整独立 target AST、一般 RAII/copy-move/runtime ABI node、结构化 import/export/chunk 与 generated-only segment 仍按 TODO 继续前移。

方阵 LIR binding 分别把 `classify_real_square`、`classify_complex_square` 与 `classify_sparse_real_square` 映射到目标自己的 structured-real-square、structured-complex-square 与 sparse-square helper。dense real helper 按 exact-value contract 依次选择 diagonal、upper/lower、pivoted tridiagonal、symmetric positive-definite Cholesky 或 dense LU；对称但非正定的候选必须回退 dense LU，右除通过转置复用同一分派。complex helper 对 exact Hermitian positive-definite 使用 Cholesky，否则使用按模选主元 dense LU；右除通过共轭转置复用求解，negative integer power 先调用同一 solver 求逆。`sparse` constructor binding 只能在已验证的 `SparseConstructionPlan` 与调用 arity 一致时选择目标 helper；JavaScript/C++ runtime 分别直接构建 canonical CSC，处理 scalar expansion、empty triplet、real/complex duplicate 求和/抵消、logical duplicate `any`、`nzmax` 与保持 complex 共轭 identity 的 sparse transpose，JavaScript 以 tagged CSC 保存 finite-real/finite-complex/logical `valueDomain`，`cpp` 分别以 `sparse_matrix<double>`、`sparse_matrix<std::complex<double>>` 与 `sparse_matrix<bool>` 保存 entry；representation planner 将 value-domain/duplicate-policy 编码为目标 integer ABI 或 helper token，renderer 不读取 plan。sparse index binding 只消费已验证的 `SparseIndexPlan`，区分 scalar 与 CSC-preserving linear/Cartesian-submatrix helper；目标 runtime 独立验证 selector、结果 shape 和 canonical CSC，`A(:)` 使用 O(nnz) 直接 remap，renderer 不从 selector 或 source shape 重建语义。sparse mutation binding 只消费已验证的 `SparseMutationPlan`，选择目标专属 assign/erase helper；runtime 对重复 selector 按列主序 last-write-wins、对 exact zero 删除 entry，并用 O(nnz + k log k) 的排序/归并事务式提交，renderer 不重建 assignment/deletion 语义。sparse solve helper 接受 canonical、静态 shape 的 real rank-two CSC coefficient（包含 `0×0`），并独立验证 column pointer、row ordering、stored values，以及 LHS/RHS/result 三组 shape ABI；非空系数走 sparse factorization，`0×0` 系数直接产生 `0×n` 或 `m×0` dense/CSC shaped-empty 结果。右除使用 sparse transpose identity，结果 storage 按源语言操作数规则确定。representation planner 将三组 shape 依调用顺序写入目标 `runtime_shape_arguments`，verifier 和 dump 重算完整 ABI，renderer 只序列化，不读取 semantic plan。矩形 binding 还必须同时匹配 real/complex numeric domain 与 CPQR factorization policy；复数路径使用 Hermitian Householder reflector，并由共轭转置右除复用一次分解。所有 helper 都拥有对应 condition/rank warning contract，JavaScript/C++ runtime 互不引用。C++ 嵌套矩阵字面量的统一数值元素类型由 representation planner 递归传播，renderer 只输出规划后的具体容器类型和 widening，不从子行重新推断类型。

Matlab shape-changing write 由 statement plan 固化为 overwrite/resize/grow/erase/overwrite-or-grow；目标 plan 同时保存线性布局、删除轴、输入/结果 shape 与 replacement conformability。JavaScript LIR 选择 checked copied-root growth/axis erase，`cpp` LIR 选择 typed nested-`std::vector` staged growth/axis erase。两者共享语义 contract，但运行时实现、renderer 和 verifier 互不引用。

Sparse reshape binding 只能消费已验证的 `SparseReshapePlan`。两个目标分别选择自己的
canonical-CSC helper，验证 input/requested/result shape 后，以列主序线性位置直接重映射 row
与 column，保持 O(nnz + result-columns)，不执行排序或 dense materialization。size vector、
dimension list、单个 `[]` 推断和请求 rank 大于 2 时的二维折叠均已在 target 分叉前固定；
representation verifier 仍须独立重算 call form，renderer 只序列化三个 shape plan。
...
- Matlab script-control label、受控 executable body 与模块级 declaration 的顺序规划；
