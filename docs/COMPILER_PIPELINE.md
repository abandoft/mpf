...
当前 LIR v7 已把函数 calling convention、临时值、作用域声明、顶层布局和当前表达式 representation canonicalization 前移。除 v6 的 `ModulePlan`/`TranslationUnitPlan`、ABI、scope/declaration 和 CSR temporary 外，每个目标 expression 现在保存强类型 form、precedence、target token/symbol、comparison strategy、direct/custom call ABI、逐实参 value/optional-forward/reference-box/copy-section、first-result 以及 element/nested/matrix/slice/row/column/block/N-D index 选择；`cpp` list 还保存 concrete recursive-vector type 与逐 child widening。独立 representation verifier 从 LIR semantic fields 重建预期计划并逐字段核对。架构门禁禁止 renderer 恢复 `ExpressionKind`、`IntrinsicId`、builtin binding、call transfer、可变 temporary allocator、声明/顶层扫描或 CLI/runtime/function-graph 解释。目标 runtime 模板已拆为独立 catalog 编译单元。完整 statement/type AST、ownership/copy-move、结构化 import/export/chunk 和 source-map segment 仍按 TODO 继续前移。
