...
> 状态说明：0.3.4 已把生产路径切换为语言专属 PMR arena AST→HIR→Analyzer `SemanticTable`→MIR→目标 semantic IR/rendered LIR→纯 emitter，并落地强类型 ID、逐层 verifier、pass/analysis、TargetProfile/legalization、opaque artifact、确定性 dump/golden、parser-session/资源 contract、extension conformance、source map v3、编译报告、fuzz 与版本化性能发布门禁。0.3.5 已让 Analyzer 预分配并直接写入按 `HirNodeId` 稠密索引、绑定 revision 的唯一 `SemanticTable`，不再注解或 move-extract HIR 语义字段；lexical scope/symbol/builtin 与 reachability/termination 分别拆为只读 HIR、revision-bound 的独立 `NameTable`/`FlowTable`，Analyzer 的确定赋值/类型状态以 `ScopeId`/`SymbolId` 稠密访问并消费 flow termination facts。参数关联的结构变化会提升 revision、同步紧凑重映射 HIR ID/facts、重建两张分析表，并重新检查资源上限。HIR→MIR 从 semantic/name facts 固化类型、shape、binding 与 symbol identity，不读取 HIR 宽语义字段。MIR alias/effect 已拆为 revision-bound、可缓存、带独立 verifier/dump 的 `AliasEffectTable`，对 storage roots、instruction read/write、函数参数摘要和 call graph 计算保守 fixed point；backend descriptor API v5 强制 capability/lowering 接收该表。静态一般 rank 的声明、RESHAPE 和直接 section 已由双后端及三维差分覆盖；tuple/function/reference 驻留类型、函数签名和显式 call-site 表也已落地，跨函数 verifier 检查 call/return、多结果、optional omission 与 OUT/INOUT writable storage。双目标 LIR v6 固化函数 ABI、CSR temporary inventory、Program/Function scope/declaration 以及 module/translation-unit layout；renderer 不再分配临时名、重扫声明、重解源 call intent，也不接收 options、runtime requirements 或函数图。剩余迁移集中在 HIR/MIR 宽兼容字段、完整 target AST/expression representation、完整官方 grammar、动态 rank/广播、精确 N 维 overlap 和插件 ABI；不能把这批架构收尾等同于完整语言兼容。
...
当前 LIR v6 已把函数 calling convention、临时值、作用域声明与顶层布局 canonicalization 前移：JavaScript 保存 module/export、value/reference-box 参数 ABI、确定性 Program/Function 声明顺序，并用 `ModulePlan` 固化 banner/directive/runtime fragment/body order；`cpp` 保存 template parameter、value/const-reference/mutable-reference/optional-reference、optional 具体类型、递归返回/前置声明 ABI、concrete/`decltype` probe、pattern path、tuple index、fixed-shape nested type，并用 `TranslationUnitPlan` 固化标准头文件、namespace、runtime fragment、forward/definition/entry order、scope placement 和 run/main 拓扑。两个目标各自使用 `LirNodeId` 索引的 CSR `offsets + slots` 保存临时角色、ordinal 和碰撞安全名称。`cpp` renderer 只为 type probe 建立一次稠密 expression view；架构门禁禁止 renderer 恢复可变 temporary allocator、声明/顶层扫描、源 call intent、CLI option、runtime requirement 或 function graph 解释。目标 runtime 模板已拆为各自独立 catalog 编译单元。完整 expression representation、runtime ABI call、结构化 import/export/chunk 和完整 target AST 仍按 TODO 继续前移。
