...
当前 LIR v4 已把函数 calling convention 与临时值 canonicalization 前移：JavaScript 保存 module/export 和 value/reference-box 参数 ABI；`cpp` 保存 template parameter、value/const-reference/mutable-reference/optional-reference、optional 具体类型及递归返回/前置声明 ABI。两个目标各自使用 `LirNodeId` 索引的 CSR `offsets + slots` 保存临时角色、ordinal 和碰撞安全名称。renderer 只能查询该计划；架构门禁禁止恢复可变 temporary allocator、读取源 call intent 或从 CLI module option现场决定 export。声明 inventory、完整 concrete representation 和结构化 target AST 仍按 TODO 继续前移。
