...
当前 LIR v5 已把函数 calling convention、临时值与作用域声明 canonicalization 前移：JavaScript 保存 module/export、value/reference-box 参数 ABI 和确定性 Program/Function 声明顺序；`cpp` 保存 template parameter、value/const-reference/mutable-reference/optional-reference、optional 具体类型、递归返回/前置声明 ABI，以及 concrete/`decltype` probe、pattern path、tuple index、fixed-shape nested type。两个目标各自使用 `LirNodeId` 索引的 CSR `offsets + slots` 保存临时角色、ordinal 和碰撞安全名称。`cpp` renderer 只为 type probe 建立一次稠密 expression view；架构门禁禁止 renderer 恢复可变 temporary allocator、声明扫描、源 call intent 或 CLI module option 解释。完整 expression representation、runtime ABI call 和结构化 target AST 仍按 TODO 继续前移。
