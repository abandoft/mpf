...
自 0.10 起，MPF 为库调用、命令行、IDE 和 CI 提供同一个诊断模型；本文索引已覆盖当前 0.34 开发分支。每条诊断包含稳定 code、severity、消息、源文件身份以及 1-based UTF-8 code-point range。未显式提供结束位置的旧诊断会覆盖起始位置的一个字符。
...
| `MPF13xx`—`MPF18xx` | 各语言 logical-source 与 statement lexer，详见下文 |
