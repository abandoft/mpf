...
using ValueDomain = mpf::detail::semantic::SparseValueDomain;
...
void configure_binary(Statement& statement, const Operation operation, const Storage left_storage,
  ...
  const auto right_shape = right_storage == Storage::none
                               ...
                               : std::vector<std::size_t>{kRows, kColumns};
  expression.sparse_arithmetic = {operation,
                                  policy,
                                  ValueDomain::finite_real,
...
void verify_sparse_arithmetic_plans(Program& program, Planner planner, Verifier verifier,
  ...
  REQUIRE((program.statements[0].expression.plan.runtime_integer_arguments ==
           std::vector<std::int64_t>{static_cast<std::int64_t>(Operation::add),
                                     ...
                                     static_cast<std::int64_t>(Storage::sparse_csc),
                                     static_cast<std::int64_t>(Storage::sparse_csc),
                                     static_cast<std::int64_t>(ValueDomain::finite_real)}));
  ...
  REQUIRE(dump.str().find("sparse-arithmetic 1 storage-policy 1 value-domain 1 storage 3,3->3") !=
          std::string::npos);
  REQUIRE(dump.str().find("sparse-arithmetic 2 storage-policy 2 value-domain 1 storage 0,3->2") !=
          std::string::npos);
  ...
  diagnostics.clear();
  planner(program);
  program.statements[0].expression.sparse_arithmetic.value_domain = ValueDomain::finite_complex;
  verifier(program, diagnostics);
  REQUIRE(!diagnostics.empty());
  ...
  program.statements[1].expression.plan.runtime_integer_arguments.back() =
      static_cast<std::int64_t>(ValueDomain::finite_complex);
  verifier(program, diagnostics);
  REQUIRE(!diagnostics.empty());
