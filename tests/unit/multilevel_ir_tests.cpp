...
TEST_CASE("Matlab sparse matrix power plans remain typed through every IR layer") {
  ...
  for (const auto& facts : analysis.semantics.expressions) {
    const auto& plan = facts.matrix_operation;
    if (plan.storage_policy != StoragePolicy::sparse_csc_power) continue;
    REQUIRE(plan.exponent_policy == ExponentPolicy::nonnegative_safe_integer);
    REQUIRE(plan.left_storage == Storage::sparse_csc);
    REQUIRE(plan.right_storage == Storage::none);
    REQUIRE(plan.result_storage == Storage::sparse_csc);
    REQUIRE(plan.left_shape == std::vector<std::size_t>({2U, 2U}));
    REQUIRE(plan.right_shape.empty());
    REQUIRE(plan.result_shape == plan.left_shape);
    REQUIRE(facts.array_storage == Storage::sparse_csc);
    REQUIRE(facts.element_type == mpf::detail::ValueType::real);
    REQUIRE(facts.element_numeric_type == mpf::detail::real_numeric_type);
  }
  ...
  REQUIRE(!mpf::detail::hir::verify_semantics(lowered.program, contradictory_hir,
                                              "sparse-power-exponent-policy-mismatch")
               .empty());

  auto mir = mpf::detail::mir::lower_from_hir(std::move(lowered.program),
                                              std::move(analysis.semantics), analysis.names);
  REQUIRE(mir.diagnostics.empty());
  REQUIRE(mpf::detail::mir::verify(mir.program, "sparse-power-plan").empty());
  REQUIRE(mpf::detail::dump_mir(mir.program)
              .find("storage-policy=5 exponent-policy=2 storage=3,0->3") != std::string::npos);
  ...
  const auto corrupt_mir = std::find_if(
      ...
      contradictory_mir.attributes.expressions.end(), [](const auto& attributes) {
        return attributes.matrix_operation.storage_policy == StoragePolicy::sparse_csc_power;
      });
  REQUIRE(corrupt_mir != contradictory_mir.attributes.expressions.end());
  corrupt_mir->matrix_operation.result_storage = Storage::dense;
...
TEST_CASE("Matlab sparse matrix products remain typed through every IR layer") {
  ...
  REQUIRE(cpp.diagnostics.empty());
  for (const auto& target_dump : {javascript.artifact->debug_dump(), cpp.artifact->debug_dump()}) {
    REQUIRE(target_dump.find("storage-policy 3 exponent-policy 0 storage 3,3->3") !=
            std::string::npos);
    REQUIRE(target_dump.find("storage-policy 3 exponent-policy 0 storage 3,2->2") !=
            std::string::npos);
...
TEST_CASE("Matlab sparse scalar products remain typed through every IR layer") {
  ...
  REQUIRE(cpp.diagnostics.empty());
  for (const auto& target_dump : {javascript.artifact->debug_dump(), cpp.artifact->debug_dump()}) {
    REQUIRE(target_dump.find("storage-policy 4 exponent-policy 0 storage 3,0->3") !=
            std::string::npos);
...
TEST_CASE("Matlab sparse reshape plans remain typed through every IR layer") {
  ...
  REQUIRE(mpf::detail::semantic::valid_sparse_reshape_contract(
      ...
      mpf::detail::ArrayStorageFormat::sparse_csc, mpf::detail::ArrayStorageFormat::sparse_csc,
      std::vector<std::size_t>{0U, 6U}, std::vector<std::size_t>{0U, 2U, 3U},
      std::vector<std::size_t>{0U, 6U}));
  REQUIRE(!mpf::detail::semantic::valid_sparse_reshape_contract(
      Kind::column_major_2d, Form::dimension_list, Inference::none, 0U,
      mpf::detail::ArrayStorageFormat::sparse_csc, mpf::detail::ArrayStorageFormat::sparse_csc,
      std::vector<std::size_t>{0U, 6U}, std::vector<std::size_t>{0U, 2U, 3U},
      std::vector<std::size_t>{0U, 5U}));
  REQUIRE(!mpf::detail::semantic::valid_sparse_reshape_contract(
  ...
  const auto zero_hir = std::find_if(analysis.semantics.expressions.begin(),
                                     analysis.semantics.expressions.end(), [](const auto& facts) {
                                       return facts.sparse_reshape.requested_shape ==
                                              std::vector<std::size_t>({0U, 2U, 3U});
                                     });
  REQUIRE(zero_hir != analysis.semantics.expressions.end());
  REQUIRE(zero_hir->sparse_reshape.input_shape == std::vector<std::size_t>({0U, 6U}));
  REQUIRE(zero_hir->sparse_reshape.result_shape == std::vector<std::size_t>({0U, 6U}));
  REQUIRE(mpf::detail::dump_semantics(analysis.semantics)
  ...
  const auto zero_mir = std::find_if(
      ...
      [&](const auto& attributes) {
        return attributes.sparse_reshape.valid() &&
               mir.program.shapes[attributes.sparse_reshape.requested_shape.value()].extents ==
                   std::vector<std::size_t>({0U, 2U, 3U});
      });
  REQUIRE(zero_mir != mir.program.attributes.expressions.end());
  REQUIRE(mir.program.shapes[zero_mir->sparse_reshape.result_shape.value()].extents ==
          std::vector<std::size_t>({0U, 6U}));
  ...
  REQUIRE(javascript.artifact->debug_dump().find(
              "sparse-reshape 1 form 2 inference 0 axis 0 input [0,6] requested [0,2,3] "
              "result [0,6]") != std::string::npos);
  REQUIRE(cpp.artifact->debug_dump().find(
              "sparse-reshape 1 form 2 inference 0 axis 0 input [0,6] requested [0,2,3] "
              "result [0,6]") != std::string::npos);
}
...
TEST_CASE("target LIR verifiers reject contradictory Matlab matrix policies") {
  ...
  const auto configure_expression = [](auto& expression) {
    ...
    expression.matrix_operation = {Operation::left_divide,
                                   ...
                                   mpf::detail::semantic::MatrixExponentPolicy::none,
                                   mpf::detail::ArrayStorageFormat::dense,
                                   mpf::detail::ArrayStorageFormat::dense,
                                   mpf::detail::ArrayStorageFormat::dense,
                                   {3U, 2U},
...
TEST_CASE("target LIR planners independently verify sparse element-wise direction") {
  using Storage = mpf::detail::ArrayStorageFormat;
  using Operation = mpf::detail::semantic::SparseElementwiseOperation;
  using Policy = mpf::detail::semantic::SparseElementwiseStoragePolicy;
  using Axis = mpf::detail::semantic::BroadcastAxis;
  const auto configure_statement = [](auto& statement, const Storage left_storage,
                                      const Storage right_storage) {
    auto& expression = statement.expression;
    expression.kind = mpf::detail::ExpressionKind::binary;
    expression.value = ".*";
    expression.operation = mpf::detail::BinaryOperator::elementwise_multiply;
    expression.inferred_type = mpf::detail::ValueType::list;
    expression.element_type = mpf::detail::ValueType::real;
    expression.element_numeric_type = mpf::detail::real_numeric_type;
    expression.array_storage = Storage::sparse_csc;
    expression.shape = {2U, 3U};
    expression.array_operation = mpf::detail::semantic::ArrayOperation::matlab;
    const auto shape_for = [](const Storage storage) {
    ...
    const auto configure_child = [](auto& child, const Storage storage, const char* name) {
      ...
      child.value = name;
      child.array_storage = storage;
      if (storage == Storage::none) {
        child.inferred_type = mpf::detail::ValueType::real;
        child.numeric_type = mpf::detail::real_numeric_type;
      } else {
        child.inferred_type = mpf::detail::ValueType::list;
        child.element_type = mpf::detail::ValueType::real;
        child.element_numeric_type = mpf::detail::real_numeric_type;
        child.shape = {2U, 3U};
      }
    };
    configure_child(expression.children[0], left_storage, "left");
    configure_child(expression.children[1], right_storage, "right");
  };
  const auto configure_program = [&](auto& program) {
    program.source_language = mpf::SourceLanguage::matlab;
    program.statements.resize(5U);
    configure_statement(program.statements[0], Storage::sparse_csc, Storage::none);
    configure_statement(program.statements[1], Storage::none, Storage::sparse_csc);
    configure_statement(program.statements[2], Storage::sparse_csc, Storage::dense);
    configure_statement(program.statements[3], Storage::dense, Storage::sparse_csc);
    configure_statement(program.statements[4], Storage::sparse_csc, Storage::sparse_csc);
  };
  ...
  javascript.statements[3].expression.plan.token = "__mpf_sparse_times_dense";
  mpf::detail::javascript::verify_lir_representation(javascript, diagnostics);
  REQUIRE(!diagnostics.empty());
  diagnostics.clear();
  javascript.statements[3].expression.plan.token = "__mpf_dense_times_sparse";
  javascript.statements[4].expression.sparse_elementwise.axes[0] = Axis::expand_left;
  mpf::detail::javascript::plan_lir_representation(javascript);
  mpf::detail::javascript::verify_lir_representation(javascript, diagnostics);
  REQUIRE(!diagnostics.empty());
  ...
  cpp.statements[0].expression.plan.sparse_elementwise.right_shape = {1U, 1U};
  mpf::detail::cpp::verify_lir_representation(cpp, diagnostics);
  REQUIRE(!diagnostics.empty());
  diagnostics.clear();
  cpp.statements[0].expression.plan.sparse_elementwise.right_shape.clear();
  cpp.statements[2].expression.sparse_elementwise.result_storage = Storage::dense;
  mpf::detail::cpp::plan_lir_representation(cpp);
  mpf::detail::cpp::verify_lir_representation(cpp, diagnostics);
  REQUIRE(!diagnostics.empty());
}
...
TEST_CASE("Matlab indexed replacement conformability remains typed through every IR layer") {
  ...
  const auto contract_count = [&](const Conformability expected, const ShapeSource shape_source) {
    return std::count_if(analysis.semantics.statements.begin(), analysis.semantics.statements.end(),
                         [&](const auto& facts) {
                           return facts.indexed_replacement.conformability == expected &&
                                  facts.indexed_replacement.shape_source == shape_source;
                         });
  };
  ...
  const auto dynamic_hir =
      std::find_if(contradictory_hir.statements.begin(), contradictory_hir.statements.end(),
                   [&](const auto& facts) {
                     return facts.indexed_replacement.shape_source == ShapeSource::runtime_values;
                   });
  REQUIRE(dynamic_hir != contradictory_hir.statements.end());
  dynamic_hir->indexed_replacement.shape_source = ShapeSource::static_extents;
  REQUIRE(!mpf::detail::hir::verify_semantics(lowered.program, contradictory_hir,
                                              "replacement-contract-corruption")
               .empty());

  auto mir = mpf::detail::mir::lower_from_hir(std::move(lowered.program),
                                              std::move(analysis.semantics), analysis.names);
  REQUIRE(mir.diagnostics.empty());
  REQUIRE(mpf::detail::mir::verify(mir.program, "replacement-contract").empty());
  const auto mir_dump = mpf::detail::dump_mir(mir.program);
  ...
  const auto dynamic_mir = std::find_if(
      ...
      contradictory_mir.attributes.statements.end(), [&](const auto& attributes) {
        return attributes.indexed_replacement.contract.shape_source == ShapeSource::runtime_values;
      });
  REQUIRE(dynamic_mir != contradictory_mir.attributes.statements.end());
  dynamic_mir->indexed_replacement.contract.shape_source = ShapeSource::static_extents;
  REQUIRE(!mpf::detail::mir::verify(contradictory_mir, "replacement-plan-corruption").empty());
...
TEST_CASE("target LIR verifiers reject corrupted Matlab indexed replacement shapes") {
  using Conformability = mpf::detail::semantic::IndexedReplacementConformability;
  using Mutation = mpf::detail::semantic::IndexedMutationKind;
  using MutationShapeSource = mpf::detail::semantic::IndexedMutationShapeSource;
  using ReplacementShapeSource = mpf::detail::semantic::IndexedReplacementShapeSource;
  using Selector = mpf::detail::semantic::IndexSelectorKind;
  const auto has_replacement_diagnostic = [](const std::vector<mpf::Diagnostic>& diagnostics) {
    return std::any_of(diagnostics.begin(), diagnostics.end(), [](const auto& diagnostic) {
      return diagnostic.message.find("indexed replacement contract") != std::string::npos;
    });
  };
  ...
  auto& javascript_statement = javascript.statements.front();
  javascript_statement.kind = mpf::detail::StatementKind::indexed_assignment;
  javascript_statement.indexed_mutation = {Mutation::overwrite, MutationShapeSource::preserve,
                                           false};
  javascript_statement.mutation_input_shape = {2, 2};
  javascript_statement.mutation_result_shape = {2, 2};
  javascript_statement.target_expression.kind = mpf::detail::ExpressionKind::index;
  javascript_statement.target_expression.inferred_type = mpf::detail::ValueType::list;
  javascript_statement.target_expression.shape = {2, 1};
  javascript_statement.target_expression.index_selectors = {Selector::slice, Selector::scalar};
  javascript_statement.expression.inferred_type = mpf::detail::ValueType::list;
  javascript_statement.expression.shape = {1, 2};
  javascript_statement.indexed_replacement = {Conformability::nonsingleton_shape,
                                              ReplacementShapeSource::static_extents};
  javascript_statement.replacement_selection_shape = {3, 1};
  javascript_statement.replacement_value_shape = {1, 2};
  mpf::detail::javascript::plan_lir_representation(javascript);
  std::vector<mpf::Diagnostic> diagnostics;
  mpf::detail::javascript::verify_lir_representation(javascript, diagnostics);
  REQUIRE(has_replacement_diagnostic(diagnostics));
  ...
  auto& cpp_statement = cpp.statements.front();
  cpp_statement.kind = mpf::detail::StatementKind::indexed_assignment;
  cpp_statement.indexed_mutation = {Mutation::overwrite, MutationShapeSource::preserve, false};
  cpp_statement.mutation_input_shape = {2, 2};
  cpp_statement.mutation_result_shape = {2, 2};
  cpp_statement.target_expression.kind = mpf::detail::ExpressionKind::index;
  cpp_statement.target_expression.inferred_type = mpf::detail::ValueType::list;
  cpp_statement.target_expression.shape = {2, 1};
  cpp_statement.target_expression.index_selectors = {Selector::slice, Selector::scalar};
  cpp_statement.expression.inferred_type = mpf::detail::ValueType::list;
  cpp_statement.expression.shape = {1, 2};
  cpp_statement.indexed_replacement = {Conformability::nonsingleton_shape,
                                       ReplacementShapeSource::static_extents};
  cpp_statement.replacement_selection_shape = {3, 1};
  cpp_statement.replacement_value_shape = {1, 2};
  mpf::detail::cpp::plan_lir_representation(cpp);
  diagnostics.clear();
  mpf::detail::cpp::verify_lir_representation(cpp, diagnostics);
  REQUIRE(has_replacement_diagnostic(diagnostics));
