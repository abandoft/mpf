...
4. Fortran case 在 gfortran 可用时以 `-std=f2023 -Wall -Wextra -Werror` 编译执行；
5. 直接比较所有可用路径，并再次与 corpus oracle 比较。

每次运行在 `build/<preset>/differential/<case>/` 保存生成源码、嵌套 C++ 构建树和 `differential-result.txt`。CI 总是上传结果文件与生成源码，失败时可直接检查路径差异。
