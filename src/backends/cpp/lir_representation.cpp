...
std::string matlab_array_helper(const lir::Expression& expression) {
  if (expression.sparse_logical.valid()) {
    switch (expression.sparse_logical.operation) {
      case semantic::SparseLogicalOperation::logical_not: return "mpf_runtime::sparse_logical_not";
      case semantic::SparseLogicalOperation::logical_and: return "mpf_runtime::sparse_logical_and";
      case semantic::SparseLogicalOperation::logical_or: return "mpf_runtime::sparse_logical_or";
  ...
  switch (expression.matrix_operation.operation) {
    case semantic::MatrixOperation::multiply:
      if (expression.matrix_operation.storage_policy ==
        ...
        if (expression.matrix_operation.left_storage == ArrayStorageFormat::sparse_csc &&
          ...
          return "mpf_runtime::sparse_scale_right";
        }
        if (expression.matrix_operation.left_storage == ArrayStorageFormat::none &&
          ...
          return "mpf_runtime::sparse_scale_left";
        }
        return {};
      }
      if (expression.matrix_operation.storage_policy ==
    ...
    case semantic::MatrixOperation::integer_power:
      if (expression.matrix_operation.storage_policy ==
          semantic::MatrixStoragePolicy::sparse_csc_power) {
        return "mpf_runtime::sparse_mpower";
      }
      return expression.matrix_operation.numeric_domain == semantic::MatrixNumericDomain::complex
                 ? "mpf_runtime::matlab_complex_mpower"
                 : "mpf_runtime::matlab_mpower";
...
std::optional<std::size_t> sparse_argument_count(const lir::Expression& expression) noexcept {
  if (expression.inferred_type != ValueType::list) {
    return expression.inferred_type == ValueType::boolean ||
                   expression.inferred_type == ValueType::integer ||
...
semantic::SparseValueDomain sparse_value_domain(const lir::Expression& expression) noexcept {
  const auto type = expression.inferred_type == ValueType::list ? expression.element_type
                                                                : expression.inferred_type;
  const auto numeric_type = expression.inferred_type == ValueType::list
...
bool valid_sparse_construction(const lir::Expression& expression) noexcept {
  ...
  if (!sparse.valid()) {
    return sparse.result_shape.empty() && sparse.triplet_element_counts.empty() &&
           sparse.reserve_hint == 0U && sparse.value_domain == semantic::SparseValueDomain::none &&
  ...
  bool valid = expression.inferred_type == ValueType::list &&
               ...
               sparse.value_domain == sparse_value_domain(expression) &&
               semantic::valid_sparse_construction_value_contract(sparse.kind, sparse.value_domain,
                                                                  sparse.duplicate_policy) &&
  ...
  if (sparse.kind == semantic::SparseConstructionKind::dense_conversion) {
    valid = valid && expression.children.size() == 2U &&
            ...
            sparse.value_domain == sparse_value_domain(expression.children[1]);
  } else if (triplets) {
    valid = valid && expression.children.size() >= 4U &&
            sparse.value_domain == sparse_value_domain(expression.children[3]);
  }
...
lir::ExpressionPlan expected_expression_plan(
  ...
  switch (expression.kind) {
    ...
    case ExpressionKind::unary:
      if (expression.unary_operation == UnaryOperator::transpose ||
        ...
        result.token = result.sparse_logical.valid() ? matlab_array_helper(expression)
                                                     : "mpf_runtime::matlab_not";
    ...
    case ExpressionKind::index: {
      ...
      const bool runtime_extent =
          std::any_of(result.index_extents.begin(), result.index_extents.end(),
                      semantic::requires_runtime_extent);
      if (std::any_of(result.index_selectors.begin(), result.index_selectors.end(),
                      semantic::selector_preserves_dimension)) {
        const bool general_selector = std::any_of(
            result.index_selectors.begin(), result.index_selectors.end(), [](const auto selector) {
              return selector != semantic::IndexSelectorKind::scalar &&
                     selector != semantic::IndexSelectorKind::slice;
            });
        if (result.index_selectors.size() == 1 && expression.column_major &&
            (runtime_extent ||
             (!expression.children.empty() && expression.children.front().shape.size() > 1U))) {
          result.index = lir::IndexForm::linear_section;
        } else if (runtime_extent || general_selector || result.index_selectors.size() > 2U) {
          result.index = lir::IndexForm::section_nd;
        } else if (result.index_selectors.size() == 1) {
          result.index = lir::IndexForm::slice;
        } else if (result.index_selectors.size() == 2) {
          if (result.index_selectors[0] == semantic::IndexSelectorKind::scalar) {
            result.index = lir::IndexForm::row_slice;
          } else if (result.index_selectors[1] == semantic::IndexSelectorKind::scalar) {
            result.index = lir::IndexForm::column;
          } else {
            result.index = lir::IndexForm::block;
          }
        }
      } else if (runtime_extent && expression.column_major && expression.children.size() == 2U) {
        result.index = lir::IndexForm::linear_element;
      } else if (runtime_extent) {
        result.index = lir::IndexForm::section_nd;
  ...
  if (result.form == lir::ExpressionForm::matlab_logical_operation ||
    ...
    if (result.sparse_logical.valid()) {
      ...
      result.runtime_integer_arguments = {
          static_cast<std::int64_t>(expression.matrix_operation.exponent_policy),
          static_cast<std::int64_t>(expression.matrix_operation.left_storage),
          static_cast<std::int64_t>(expression.matrix_operation.result_storage)};
    } else if (expression.matrix_operation.storage_policy ==
                   semantic::MatrixStoragePolicy::sparse_csc_multiply ||
...
void verify_expression(const lir::Expression& expression, const lir::EmissionPlan& emission,
  ...
  if (expression.reduction.operation != expected_reduction) {
    add_error(diagnostics, expression.location,
              "cpp LIR logical reduction identity is inconsistent");
  ...
  if (expression.sparse_index.kind != expected_sparse_index) {
    add_error(diagnostics, expression.location, "cpp LIR sparse-index identity is inconsistent");
    ...
    add_error(diagnostics, expression.location, "cpp LIR inactive sparse-index retains state");
  }
  if ((expression.kind == ExpressionKind::end_index) !=
      semantic::requires_runtime_extent(expression.index_extent)) {
    add_error(diagnostics, expression.location,
              "cpp LIR runtime index-extent ownership is inconsistent");
  }
  if (expression.kind == ExpressionKind::index) {
    if (expression.index_selectors.size() + 1U != expression.children.size() ||
      ...
      add_error(diagnostics, expression.location,
                "cpp LIR index selector or extent arity is inconsistent");
    } else {
      for (std::size_t index = 0; index < expression.index_selectors.size(); ++index) {
        ...
        if (expected.has_value() && *expected != expression.index_selectors[index]) {
          add_error(diagnostics, expression.location,
                    "cpp LIR index selector identity is inconsistent");
          break;
        }
        ...
        if (!valid_extent ||
          ...
          add_error(diagnostics, expression.location,
                    "cpp LIR runtime index-extent plan is inconsistent");
          break;
        }
      }
    }
    ...
    add_error(diagnostics, expression.location,
              "cpp non-index LIR expression retains selector or extent facts");
  }
  ...
  if (!valid_sparse_logical_plan(expression)) {
    add_error(diagnostics, expression.location, "cpp LIR sparse logical plan is inconsistent");
  }
  ...
  if (expression.matrix_operation.operation != expected_matrix ||
      (!expression.matrix_operation.valid() &&
       (expression.matrix_operation.solve != semantic::MatrixSolveKind::none ||
        ...
        expression.matrix_operation.exponent_policy != semantic::MatrixExponentPolicy::none ||
  ...
  if (expression.sparse_elementwise.valid() && expression.plan.token.empty()) {
    add_error(diagnostics, expression.location,
              "cpp LIR sparse element-wise plan has no target helper");
  }
  if (expression.sparse_logical.valid() && expression.plan.token.empty()) {
    add_error(diagnostics, expression.location, "cpp LIR sparse logical plan has no target helper");
  }
  if (!valid_sparse_construction(expression)) {
    add_error(diagnostics, expression.location, "cpp LIR sparse-construction plan is inconsistent");
  }
  if (!valid_sparse_reshape(expression)) {
    add_error(diagnostics, expression.location, "cpp LIR sparse-reshape plan is inconsistent");
  }
  ...
  switch (statement.kind) {
    ...
    case StatementKind::indexed_assignment:
      result.form = statement.target_expression.plan.index == lir::IndexForm::slice ||
                            ...
                            statement.target_expression.plan.index == lir::IndexForm::linear_section
          ...
          (statement.indexed_replacement.valid() &&
           (!semantic::same_indexed_replacement_contract(statement.indexed_replacement,
            ...
            !semantic::valid_indexed_replacement_contract(statement.indexed_replacement,
                                                          ...
                                                          statement.replacement_value_shape)))) {
        add_error(diagnostics, {statement.line, 1},
                  "cpp LIR indexed replacement contract is inconsistent");
      }
