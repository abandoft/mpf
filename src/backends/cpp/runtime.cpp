  ...
  void emit(const std::string_view runtime_namespace,
    ...
    output_
        << "template <typename T> class optional_argument {\n"
           ...
           "  return result;\n"
           "}\n"
           "template <typename Values, typename Mask, std::size_t ValueRank, "
           "std::size_t MaskRank> auto logical_index_nd("
           "const Values& values, const Mask& mask, "
           "const std::array<std::size_t, ValueRank>& shape, "
           "const std::array<std::size_t, MaskRank>& mask_shape) {\n"
           "  std::vector<scalar_type_t<Values>> result;\n"
           "  const auto size = shape_size(shape);\n"
           "  if (size != shape_size(mask_shape)) throw std::invalid_argument("
           "\"MPF Matlab logical mask shape mismatch\");\n"
           "  for (std::size_t linear = 0; linear < size; ++linear) {\n"
           "    const auto coordinates = column_major_coordinates(linear, shape);\n"
           "    const auto mask_coordinates = column_major_coordinates(linear, mask_shape);\n"
           "    if (static_cast<bool>(nd_value(mask, mask_coordinates))) "
           "result.push_back(nd_value(values, coordinates));\n"
           "  }\n"
           "  return result;\n"
           "}\n"
           "template <typename Values, typename Mask, typename Replacement, "
           "std::size_t ValueRank, std::size_t MaskRank> "
           "void assign_logical_nd(Values& values, const Mask& mask, const Replacement& "
           "replacement, const std::array<std::size_t, ValueRank>& shape, "
           "const std::array<std::size_t, MaskRank>& mask_shape) {\n"
           ...
           "    const auto mask_coordinates = column_major_coordinates(linear, mask_shape);\n"
           "    if (static_cast<bool>(nd_value(mask, mask_coordinates))) "
           "selected.push_back(linear);\n"
           "  }\n"
           "  if constexpr (is_vector<std::decay_t<Replacement>>::value) {\n"
           "    if (replacement.size() != selected.size()) throw std::invalid_argument("
           "\"MPF Matlab logical assignment size mismatch\");\n"
           "    for (std::size_t index = 0; index < selected.size(); ++index) "
           "nd_set(values, column_major_coordinates(selected[index], shape), "
           "replacement[index]);\n"
           "  } else {\n"
           "    for (const auto linear : selected) nd_set(values, "
           "column_major_coordinates(linear, shape), replacement);\n"
           "  }\n"
           "}\n"
