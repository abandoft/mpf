...
LirExpression lower_lir_expression(const mir::Expression& source, IrIdAllocator<LirNodeId>& ids,
  ...
  for (const auto& child : source.children) {
    result.children.push_back(lower_lir_expression<LirExpression>(child, ids, resolve_binding));
  }
  ...
  result.argument_intents = source.argument_intents;
  result.argument_names = source.argument_names;
  result.argument_optional_forward = source.argument_optional_forward;
...
LirSelector lower_lir_selector(const mir::CaseSelector& source, IrIdAllocator<LirNodeId>& ids,
                               const ResolveBinding& resolve_binding) {
  LirSelector result;
  result.lower = lower_lir_expression<LirExpression>(source.lower, ids, resolve_binding);
  result.has_lower = source.has_lower;
  result.upper = lower_lir_expression<LirExpression>(source.upper, ids, resolve_binding);
...
LirStatement lower_lir_statement(const mir::Statement& source, IrIdAllocator<LirNodeId>& ids,
  ...
  result.expression = lower_lir_expression<LirExpression>(source.expression, ids, resolve_binding);
  result.has_expression = source.has_expression;
  result.procedure_call = source.procedure_call;
  result.secondary_expression =
      lower_lir_expression<LirExpression>(source.secondary_expression, ids, resolve_binding);
  result.has_secondary_expression = source.has_secondary_expression;
  result.tertiary_expression =
      lower_lir_expression<LirExpression>(source.tertiary_expression, ids, resolve_binding);
  ...
  result.target_expression =
      lower_lir_expression<LirExpression>(source.target_expression, ids, resolve_binding);
  ...
  for (const auto& expression : source.parameter_defaults) {
    result.parameter_defaults.push_back(
        lower_lir_expression<LirExpression>(expression, ids, resolve_binding));
  }
  ...
  for (const auto& selector : source.case_selectors) {
    result.case_selectors.push_back(
        lower_lir_selector<LirSelector, LirExpression>(selector, ids, resolve_binding));
  }
