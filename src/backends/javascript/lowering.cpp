  ...
  if (expression.kind == ExpressionKind::call && !expression.children.empty()) {
    ...
    if (callee != nullptr && callee_attributes != nullptr &&
        ...
        (callee_attributes->intrinsic == IntrinsicId::complex_value ||
         ...
         (callee_attributes->intrinsic == IntrinsicId::absolute &&
          expression.children.size() == 2U &&
          mir::expression(program, expression.children[1]) != nullptr &&
          mir::numeric_type(program, mir::expression(program, expression.children[1])->type_id)
                  .complexity == NumericComplexity::complex))) {
      result.runtime.require(lir::RuntimeFeature::complex_numbers);
    }
    if (callee != nullptr && callee_attributes != nullptr &&
        callee_attributes->binding == BindingKind::builtin &&
...
void verify_expression(const lir::Expression& expression, const std::size_t node_count,
  ...
  if (expression.argument_boundaries.size() != expression.argument_transfers.size()) {
    add_error(diagnostics, expression.location,
              "JavaScript LIR call has an invalid boundary-conversion plan");
  }
  for (std::size_t index = 0; index < expression.argument_transfers.size(); ++index) {
    ...
    if (!valid_argument_call_boundary(boundary)) {
      add_error(diagnostics, expression.location,
                "JavaScript LIR call has a malformed boundary-conversion plan");
    }
    if (transfer == ArgumentTransfer::omitted && !(boundary == ArgumentCallBoundary{})) {
      add_error(diagnostics, expression.location,
                "JavaScript LIR omitted argument retains a boundary conversion");
    }
