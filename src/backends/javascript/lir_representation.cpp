...
std::string matlab_array_helper(const lir::Expression& expression) {
  ...
  if (expression.sparse_arithmetic.valid()) {
    switch (expression.sparse_arithmetic.operation) {
      case semantic::SparseArithmeticOperation::add: return "__mpf_sparse_add";
      case semantic::SparseArithmeticOperation::subtract: return "__mpf_sparse_subtract";
...
void verify_expression(const lir::Expression& expression, const lir::EmissionPlan& emission,
  ...
  if (expression.exception.operation != expected_exception) {
    add_error(diagnostics, expression.location,
              "JavaScript LIR exception identity is inconsistent");
    ...
    if (!semantic::valid_exception_contract(expression.exception.operation,
      ...
      add_error(diagnostics, expression.location,
                "JavaScript LIR exception result, message, or stack contract is inconsistent");
    }
    ...
    add_error(diagnostics, expression.location,
              "JavaScript LIR inactive exception operation retains state");
  }
  ...
  if (!valid_sparse_arithmetic_plan(expression)) {
    add_error(diagnostics, expression.location,
              "JavaScript LIR sparse arithmetic plan is inconsistent");
  }
  ...
  if (expression.sparse_arithmetic.valid() && expression.plan.token.empty()) {
    add_error(diagnostics, expression.location,
              "JavaScript LIR sparse arithmetic plan has no target helper");
  }
...
lir::StatementPlan expected_statement_plan(const lir::Statement& statement,
  ...
  switch (statement.kind) {
    ...
    case StatementKind::return_statement:
      ...
      } else if (source_language == SourceLanguage::matlab && !in_function) {
...
void plan_statements(std::vector<lir::Statement>& statements, const lir::EmissionPlan& emission,
  ...
  for (auto& statement : statements) {
    ...
    statement.plan =
        expected_statement_plan(statement, emission, context, source_language, in_function);
