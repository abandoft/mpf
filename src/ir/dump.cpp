...
std::string dump_semantics(const hir::SemanticTable& table) {
  std::ostringstream output;
  output << "semantic-v32 hir-nodes=" << table.hir_node_count
...
std::string dump_mir(const mir::Program& program) {
  std::ostringstream output;
  output << "mir-v38 language=" << enum_value(program.source_language)
  ...
  for (std::size_t index = 1; index < program.statements.size(); ++index) {
    ...
    if (attributes != nullptr) {
      ...
      if (attributes->indexed_replacement.valid()) {
        output << " conformability="
               << enum_value(attributes->indexed_replacement.contract.conformability)
               << " replacement-shape-source="
               << enum_value(attributes->indexed_replacement.contract.shape_source)
               << " selection=!s" << attributes->indexed_replacement.selection_shape.value()
               << " value=!s" << attributes->indexed_replacement.value_shape.value();
      }
