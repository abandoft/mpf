...
std::optional<semantic::IndexSelectorKind> expected_index_selector(
    const Program& program, const Expression& selector) noexcept {
  if (selector.kind == ExpressionKind::slice) return semantic::IndexSelectorKind::slice;
  const auto selector_type = value_type(program, selector.type_id);
  if (selector_type == ValueType::unknown) return std::nullopt;
  if (selector_type != ValueType::list) return semantic::IndexSelectorKind::scalar;
  const auto* selector_shape = shape(program, selector.shape_id);
  if (selector_shape != nullptr &&
      std::find(selector_shape->extents.begin(), selector_shape->extents.end(), 0U) !=
          selector_shape->extents.end()) {
    return semantic::IndexSelectorKind::empty;
  }
  const auto selector_element = element_type(program, selector.type_id);
  if (selector_element == ValueType::boolean) return semantic::IndexSelectorKind::logical;
  if (selector_element == ValueType::integer || selector_element == ValueType::real) {
...
semantic::SparseReshapeKind expected_sparse_reshape_kind(const Program& program,
                                                         const Expression& expression) noexcept {
  if (expression.kind != ExpressionKind::call || expression.children.size() < 3U) {
    return semantic::SparseReshapeKind::none;
  }
  const auto* callee = mir::expression(program, expression.children.front());
  const auto* source = mir::expression(program, expression.children[1]);
  const auto* callee_attributes = callee == nullptr ? nullptr : attributes(program, callee->id);
  return callee_attributes != nullptr && source != nullptr &&
                 callee_attributes->binding == BindingKind::builtin &&
                 callee_attributes->intrinsic == IntrinsicId::reshape &&
                 array_storage(program, source->type_id) == ArrayStorageFormat::sparse_csc
...
semantic::MatrixOperation expected_matrix_operation(
  ...
  if (program.source_language != SourceLanguage::matlab ||
      attributes.array_operation != semantic::ArrayOperation::matlab) {
  ...
  const bool left_array =
      value_type(program, program.expressions[expression.children[0].value()].type_id) ==
      ValueType::list;
  ...
  if (attributes.operation == BinaryOperator::divide && left_array && right_array) {
    return semantic::MatrixOperation::right_divide;
  }
  if (attributes.operation == BinaryOperator::power && left_array && !right_array) {
...
bool static_rank_two(const ShapeData* shape_data) noexcept {
  return shape_data != nullptr && shape_data->extents.size() == 2U &&
         std::find(shape_data->extents.begin(), shape_data->extents.end(), dynamic_extent) ==
             shape_data->extents.end();
}

bool valid_matrix_shapes(const Program& program, const MatrixOperationPlan& plan,
      ...
      return plan.solve == semantic::MatrixSolveKind::none && static_rank_two(right) &&
             ...
             (plan.storage_policy != semantic::MatrixStoragePolicy::sparse_csc_multiply ||
              (plan.numeric_domain == semantic::MatrixNumericDomain::real &&
               std::find(left->extents.begin(), left->extents.end(), 0U) == left->extents.end() &&
               std::find(right->extents.begin(), right->extents.end(), 0U) ==
                   right->extents.end())) &&
...
void verify_expression(const Expression& expression, const Program& program,
  ...
  if (expression.kind == ExpressionKind::index) {
    if (expression_attributes->index_selectors.size() + 1U != expression.children.size() ||
      ...
      for (std::size_t selector_index = 0;
        ...
        const auto* selector = mir::expression(program, expression.children[selector_index + 1U]);
        const auto expected = selector == nullptr ? std::optional<semantic::IndexSelectorKind>{}
                                                  : expected_index_selector(program, *selector);
        if (expected.has_value() &&
            *expected != expression_attributes->index_selectors[selector_index]) {
          add_error(diagnostics, expression.location, stage,
  ...
  const auto expected_matrix =
      expected_matrix_operation(program, expression, *expression_attributes);
  ...
  const auto& sparse_reshape = expression_attributes->sparse_reshape;
  const auto expected_sparse_reshape = expected_sparse_reshape_kind(program, expression);
  if (sparse_reshape.kind != expected_sparse_reshape) {
    add_error(diagnostics, expression.location, stage,
              "sparse-reshape identity disagrees with its MIR source intrinsic");
  } else if (sparse_reshape.valid()) {
    const auto* source = mir::expression(program, expression.children[1]);
    const auto* input_shape = shape(program, sparse_reshape.input_shape);
    const auto* requested_shape = shape(program, sparse_reshape.requested_shape);
    const auto* result_shape = shape(program, sparse_reshape.result_shape);
    ...
    if (expected_form == semantic::SparseReshapeDimensionForm::dimension_list) {
      for (std::size_t child = 2U; child < expression.children.size(); ++child) {
        const auto* dimension = mir::expression(program, expression.children[child]);
        if (dimension != nullptr && dimension->kind == ExpressionKind::list &&
            dimension->children.empty()) {
          empty_axis = child - 2U;
          ++empty_dimensions;
        }
      }
    ...
    const bool valid =
        ...
        (expected_inference == semantic::SparseReshapeInference::none ||
         sparse_reshape.inferred_axis == empty_axis) &&
        sparse_reshape.input_shape == source->shape_id &&
        sparse_reshape.result_shape == expression.shape_id &&
        sparse_reshape.source_storage == array_storage(program, source->type_id) &&
        sparse_reshape.result_storage == array_storage(program, expression.type_id) &&
        semantic::valid_sparse_reshape_contract(
            sparse_reshape.kind, sparse_reshape.dimension_form, sparse_reshape.inference,
            sparse_reshape.inferred_axis, sparse_reshape.source_storage,
            sparse_reshape.result_storage, input_shape->extents, requested_shape->extents,
            result_shape->extents);
    if (!valid) {
      add_error(diagnostics, expression.location, stage,
                "sparse-reshape attributes have an invalid type, syntax, storage, or shape");
    }
  } else if (sparse_reshape.dimension_form != semantic::SparseReshapeDimensionForm::none ||
             sparse_reshape.inference != semantic::SparseReshapeInference::none ||
             sparse_reshape.inferred_axis != 0U ||
             sparse_reshape.source_storage != ArrayStorageFormat::none ||
             sparse_reshape.result_storage != ArrayStorageFormat::none ||
             sparse_reshape.input_shape.valid() || sparse_reshape.requested_shape.valid() ||
             sparse_reshape.result_shape.valid()) {
    add_error(diagnostics, expression.location, stage,
              "inactive sparse-reshape attributes retain MIR state");
  }
  ...
  for (std::size_t index = 1; index < program.statements.size(); ++index) {
    ...
    if (statement_attributes == nullptr || statement_attributes->origin != statement.id) {
      ...
      if (statement.kind == StatementKind::indexed_assignment) {
        ...
        const bool expected_sparse = target != nullptr && target_attributes != nullptr &&
                                     target_attributes->sparse_index.valid() &&
                                     replacement != nullptr;
        if (expected_sparse != sparse.valid()) {
          ...
          const auto expected_kind =
              deletion
                  ...
                  : (mutation.contract.linear ? semantic::SparseMutationKind::linear_assignment
          ...
          if (sparse_input == nullptr || sparse_selection == nullptr ||
              ...
              target_selection == nullptr || sparse.kind != expected_kind ||
              sparse_input->extents != generic_input->extents ||
              ...
              !semantic::valid_sparse_mutation_contract(
                  ...
                  sparse_input->extents, sparse_selection->extents, sparse_replacement->extents,
                  sparse_result->extents, target->children.size() - 1U, mutation.contract)) {
            add_error(diagnostics, {statement.line, 1}, stage,
                   ...
                   sparse.input_shape.valid() || sparse.selection_shape.valid() ||
                   sparse.replacement_shape.valid() || sparse.result_shape.valid()) {
          add_error(diagnostics, {statement.line, 1}, stage,
