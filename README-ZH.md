...
| 输入语言 | 自动识别扩展名 | 当前能力摘要 |
|---|---|---|
| Matlab | `.m` | 脚本与局部函数、条件/循环/标量 `switch`、矩阵、静态 N 维隐式扩展、数组比较、`end`、保序/重复/空 numeric selector 与线性/逐维 logical selector、转置、section、`reshape`、二维矩阵乘法、静态满秩稠密实数方阵/矩形求解、方阵整数幂与多输出函数 |
