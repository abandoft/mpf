...
本路线图记录 **0.7.2 当前开发基线** 与后续交付目标的真实状态。历史交付细节见
...
| 工程门禁 | 298 项内部测试；106 个差分 case、257 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 均为 153 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、36 项生成 runtime 拒绝测试、发布脚本正/负契约、39 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [ ] Matlab 商业发布 P0-A：依据 R2024a/R2024b grammar 补齐高频 command/function 调用、`return`、`try/catch`、`arguments` block 与产生式级 recovery/诊断；未实现的 class/parallel/toolbox 产生式必须精确失败关闭
