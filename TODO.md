...
本路线图记录 **0.7.5 当前开发基线** 与后续交付目标的真实状态。历史交付细节见
...
| IR 架构 | 四种语言使用编译期互不兼容的 PMR arena AST，并原子 lowering 到窄 HIR v3 与 revision-checked 稠密 semantic side table；名称、控制流、alias/effect 和 memory-dependence 分别由独立表持有。MIR v37 使用强类型稠密 expression/statement/instruction arena、显式普通/异常 CFG、resident instruction、`ExceptionRegion` 与 revision-bound attributes；Semantic v31→MIR v37→双目标 LIR v46 逐层验证 numeric class/complexity、dense/CSC array storage、matrix numeric domain/solve/condition/factorization/structure/storage/exponent policy、包含 finite-complex 的 sparse construction、arithmetic 与 matrix-product 值域、重复项策略、index/mutation/reshape/product/scale/power、独立 `SparseArithmeticPlan`/`SparseElementwisePlan`/`SparseLogicalPlan` 与带输入/结果 storage policy 的 `ReductionPlan`、storage region、call ownership/writeback、Matlab command 隐式结果/前序确定赋值、异常 handler/catch storage、目标 runtime shape/integer 调用 ABI、函数输出/脚本退出形式和 source segment plan。JavaScript LIR 与 `cpp` LIR 各自完成 representation/runtime/module/exception 规划，emitter 仅序列化，任一目标都不读取另一目标产物。 |
...
| 工程门禁 | 312 项内部测试；109 个差分 case、263 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 均为 156 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、36 项生成 runtime 拒绝测试、发布脚本正/负契约、42 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [ ] Matlab 商业发布 P0-B：建立可跨函数传播不可结构恢复 shape 的统一动态 NDArray ABI，完成一般 assignment conformability、rank/shape/stride/layout、值语义和 owner/view 边界
