...
本路线图记录 **0.5.2 当前开发基线** 与后续交付目标的真实状态。历史交付细节见
...
| IR 架构 | 四种语言使用编译期互不兼容的 PMR arena AST；AST→HIR 原子产出窄结构 HIR 与 revision-checked 稠密 `SemanticTable` seed，HIR 节点不再镜像 type/shape/binding/call/assignment facts；名称/作用域与控制流分别由 `NameTable`、`FlowTable` 持有，profile 驱动的 `NameScopeEdges` 为 function/statement/body/alternative 建立稠密 scope graph；MIR v13 使用 `MirExpressionId`/`MirStatementId` 稠密 arena、revision-bound `OperationAttributeTable` 与显式 retired tombstone，结构节点不再镜像宽语义 payload；同一属性表按 `InstructionId` 稠密保存零到多个 storage/root/region/mode `MemoryAccess`，指令压缩同步重映射；conditional/短路/comparison chain 和 TypeScript canonical `for` 已产生显式 CFG、typed edge merge 和 runtime-independent store；默认共享 pass 已接通 shape canonicalization、相同 edge-actual copy propagation、精确整数/布尔 constant folding、dead-pure elimination 和保守 CFG cleanup，每个 pass 后验证并同步 revision；tuple/function/reference type/shape 签名、stride/view/lifetime 与 call argument transfer 可验证；Matlab 广播的 static-extents/runtime-operands shape source、逐轴 mode、每个下标的 selector identity、runtime-axis/runtime-linear extent source、direct/shaped-empty array literal，以及 overwrite/resize/grow/erase `IndexedMutationContract` 由 Semantic v8、MIR v13 与双目标 LIR v19 逐层验证；shape-changing write 统一写整个 storage root，静态已知 shape 的 identifier/element/N 维矩形及列主序线性 section 则由 HIR/MIR `StorageRegion` side table 规范化，alias/effect v3 在优化后统一直接访问与跨函数 actual-region 实例化并提供访问级冲突查询；`MemoryDependenceTable` v1 在函数 CFG 上建立 region-refined flow/anti/output、unknown barrier 与 loop-carried adjacency，函数 write summary 会裁剪不可能参与 WAR 的纯 read frontier，full-root write 在形成必要 hazard 后裁剪被覆盖的同根历史，并拥有独立 revision、verifier、dump、cache 和报告；双目标 LIR v19 以 `SymbolId` 保存名称身份，并显式保存 lexical scope/declaration、ABI、source export、临时值、顶层拓扑、expression/statement、强类型比较、Matlab array-literal/broadcast/matrix-operation/solve/selector/extent/mutation plan、call ownership/writeback/evaluation 与稠密 source segment plan，emitter 仅序列化 |
...
| 工程门禁 | 212 项内部测试；69 个差分 case、183 条工具完整环境执行路径；Release/RelWithDebInfo 84 项、Debug 83 项 CTest；四语言 fuzz smoke、可选 libFuzzer、3 项生成 runtime 拒绝测试、发布脚本正/负契约、仅优化配置启用的版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [ ] 继续 Matlab P0：秩亏求解兼容策略、complex/numeric class、可跨函数传播 shape 的统一动态 NDArray ABI，以及多轴或非 vector 线性删除语义
...
- [x] indexed deletion 与 growth；非 vector matrix 的线性删除、多轴删除和缺失维度的 shape-changing assignment 以 `MPF2050` 失败关闭
- [ ] 元胞数组、struct、string、table、datetime 等核心类型
- [ ] 秩亏矩阵求解的 Matlab 兼容行为、非整数矩阵幂、numeric class、complex、稀疏数组，以及一般 NDArray 值语义
