...
本路线图记录 **0.7.8 当前源码状态** 与后续交付目标。历史交付细节见
...
| IR 架构 | 四种语言使用编译期互不兼容的 PMR arena AST，并原子 lowering 到窄 HIR v3 与 revision-checked 稠密 semantic side table；名称、控制流、alias/effect 和 memory-dependence 分别由独立表持有。MIR v40 使用强类型稠密 expression/statement/instruction arena、显式普通/异常 CFG、resident instruction、`ExceptionRegion` 与 revision-bound attributes；Semantic v34→MIR v40→双目标 LIR v49 逐层验证 numeric class/complexity、dense/CSC array storage、matrix numeric domain/solve/condition/factorization/structure/storage/exponent policy、包含 finite-complex 的 sparse construction、arithmetic 与 matrix-product 值域、重复项策略、index/mutation/reshape/product/scale/power、Matlab runtime selector、overwrite-or-grow mutation 与 indexed-replacement conformability、独立 `SparseArithmeticPlan`/`SparseElementwisePlan`/`SparseLogicalPlan` 与带输入/结果 storage policy 的 `ReductionPlan`、storage region、call ownership/writeback、Matlab command 隐式结果/前序确定赋值，以及异常 operation/message-form/stack-policy、handler/catch storage、目标 runtime shape/integer 调用 ABI、函数输出/脚本退出形式和 source segment plan。JavaScript LIR 与 `cpp` LIR 各自完成 representation/runtime/module/exception 规划，emitter 仅序列化，任一目标都不读取另一目标产物。 |
...
| Matlab sparse 归约 | 静态 real/logical rank-2 CSC 的 `all`/`any` 支持默认维度、常量 `dim`/`vecdim`、`'all'`、高于 rank 的 no-op 维度和零 extent；非标量结果保持 canonical logical CSC，全维结果返回 full logical scalar，两个目标均按需装载独立 O(nnz + output-extent) runtime，不物化 dense source |
| Matlab complex sparse arithmetic | 静态 rank-2 complex CSC 支持 compatible-size `+`/`-`；sparse-sparse 保持 canonical complex CSC，real/logical operand 自动提升，mixed dense/scalar 物化 complex dense，支持 singleton row/column expansion、零 extent、exact-zero cancellation 与双向 complex scalar |
...
| Matlab control/command | 函数与脚本裸 `return` 已进入名称、flow、MIR 和双目标 LIR；函数早退保持声明的单/多输出，JavaScript 脚本使用碰撞安全的目标控制区，C++ 从生成入口返回；通用 command syntax 可调用受支持的本地函数/内建函数，参数按 character vector 与 Matlab 引号/空格规则解析，未被变量/参数遮蔽的零输入 local function 还可使用裸调用，产值写入 `ans`，无输出调用保持既有 `ans`，`disp`/`display` 严格接受一个参数；`try`/单一 `catch [exception]`、嵌套 handler、格式化 `error`、`MException`、`throw`/`throwAsCaller`/`rethrow`、`addCause`、`getReport` 和异常 `identifier`/`message` 已贯通显式异常 CFG 与双目标独立 runtime |
...
| 工程门禁 | 319 项内部测试；113 个差分 case、271 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 均为 167 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、43 项生成 runtime 拒绝测试、发布脚本正/负契约、45 项独立版本化通用及 Matlab 专项性能场景、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [x] Matlab P0 shape mutation：overwrite/resize/grow/erase/overwrite-or-grow 强类型 contract 贯穿 Semantic/HIR/MIR/双目标 LIR；覆盖 vector、matrix、一般 N 维多轴扩容与单轴删除、列主序线性扩容、运行时 scalar/numeric/logical selector、动态 range、空值填充、source map、差分、fuzz、性能和损坏事实拒绝
...
- [x] Matlab P0 condition-aware 求解：矩形 `\`/`/` 使用列主元 QR 基本最小二乘解并在秩亏时警告；方阵使用部分主元 LU、迭代 1-范数 `rcond` 估计，在精确奇异/近奇异时分别警告并继续产生 IEEE 结果；`MatrixConditionPolicy` 当前贯穿 Semantic v20/MIR v26/双目标 LIR v32
- [x] Matlab P0 首批结构感知方阵分派：`MatrixStructurePolicy` 当前贯穿 Semantic v20/MIR v26/双目标 LIR v32；exact-zero diagonal/upper/lower 使用直接或三角求解及对应条件估计，其余方阵回退 dense LU，左右除、source map、差分、fuzz、性能与损坏事实拒绝均进入门禁
- [x] Matlab P0 高级实数方阵分派：`classify_real_square` 当前贯穿 Semantic v20/MIR v26/双目标 LIR v32；full tridiagonal 使用相邻行部分主元紧凑 LU，非三对角 exact-symmetric positive-definite 使用 Cholesky，对称不定安全回退 dense LU，正/转置求解、条件 warning、左右除、source map、差分、fuzz 和性能均进入门禁
- [x] Matlab P0 逻辑纵切面：lexer/AST 分离 `&`/`|` 与 `&&`/`||` 并遵循官方 precedence；`LogicalEvaluation` 当前在 Semantic v20/MIR v26/双目标 LIR v32 固化 eager-elementwise/boolean-short-circuit/operand-short-circuit，`~`/`&`/`|` 支持 compatible-size N 维逻辑值，condition 使用非空且全元素非零规则；双目标 runtime、source map、拒错、差分、fuzz 和性能门禁完成
- [x] Matlab P0 逻辑归约纵切面：`all`/`any` 绑定为 Matlab 专属 intrinsic；`ReductionPlan` 当前在 Semantic v20/MIR v26/双目标 LIR v32 固化 operation、默认/显式/全维 axis policy、static/runtime shape source、输入/输出 shape 与 scalar identity；默认首个非 singleton 维、常量 `dim`/`vecdim`、`'all'`、N 维及零 extent identity 已由双目标独立列主序 runtime、source map、拒错、差分、fuzz 和第二十一项性能场景验证
- [x] 商业级标量除法纵切面：`Division` 与 `DivisionByZero` 当前经 HIR v2/MIR v26/双目标 LIR v32 传播并独立验证；Python true/floor division 使用双目标 checked runtime，Matlab/TypeScript C++ 使用 portable IEEE helper，renderer 仅序列化 target plan；MSVC、惰性零除、损坏合同和两项生成 runtime 拒错进入门禁
- [x] Matlab P0 complex 基础纵切面：`NumericClass`/`NumericComplexity` 权威 side table 贯穿语言 AST、HIR、Semantic v20、MIR v26 与双目标 LIR v32；支持 imaginary literal、可遮蔽 `i`/`j`、complex scalar/array 逐元素语义、索引写入/reshape、共轭/普通转置及跨函数动态 numeric 分派；JavaScript object ABI 与 C++ `std::complex<double>` runtime 完全独立，并覆盖损坏事实拒绝、source map、差分、fuzz、feature pruning 和第二十二项性能场景
- [x] Matlab P0 complex 稠密方阵纵切面：`MatrixNumericDomain::complex` 与 `classify_complex_square` 贯穿 Semantic v20/MIR v26/双目标 LIR v32；二维矩阵乘法、Hermitian 正定 Cholesky、按模选主元 dense LU、共轭转置右除、singular/nearly-singular warning 及 safe-integer 正/零/负方阵幂由独立目标 runtime、source map、差分、fuzz、拒错和第二十三项性能场景验证
- [x] Matlab P0 complex 矩形求解纵切面：`MatrixFactorizationPolicy::rank_revealing_column_pivoted_qr` 贯穿 Semantic v20/MIR v26/双目标 LIR v32；超定/欠定、多 RHS、秩亏 basic solution 与 warning、共轭转置右除由 JavaScript/C++ 独立 runtime、source map、差分、fuzz、损坏事实拒绝和第二十四项性能场景验证
- [x] Matlab P0 sparse CSC 方阵求解纵切面：`ArrayStorageFormat` 与 `MatrixStoragePolicy::sparse_csc_coefficient` 贯穿语言 AST、Semantic v21、MIR v27 与双目标 LIR v34；`sparse(A)`/`full`/`issparse`/`nnz`、三对角 O(nnz) 快路、一般稀疏行主元 LU、正/转置条件估计、左右除、`0×0 \ 0×n` 与 `m×0 / 0×0` 的 dense/CSC shaped-empty 结果由独立 JavaScript/C++ runtime 实现；目标 `runtime_shape_arguments` 明确固化三个 shape 实参，renderer 只序列化该 ABI，并进入 source map、差分、fuzz、生成计划拒错、损坏事实和第二十五项性能门禁
- [x] Matlab P0 sparse constructor/transpose 纵切面：R2024 的五类 `sparse` 调用形式由 `SparseConstructionPlan` 在 Semantic v20、MIR v26 和双目标 LIR v32 固化 arity、result shape、triplet cardinality 与 reserve hint；zero/empty、推断/显式 shape、scalar expansion、重复项累加与抵消、`nzmax`、real CSC 普通/共轭转置由双目标直接 CSC runtime、source map、差分、生成拒错、fuzz、架构、性能和逐层损坏事实拒绝验证
- [x] Matlab P0 sparse indexing 纵切面：`SparseIndexPlan` 在 Semantic v16、MIR v22 和双目标 LIR v28 固化 linear/subscript scalar 与 storage-preserving selection identity、输入/结果 shape 和 storage；支持 scalar、colon、slice、保序/重复/空 numeric 与 logical selector，双下标按 Cartesian product 取子矩阵，`A(:)` 使用 O(nnz) 直接 CSC 快路，静态零 extent source/selector 保持 shaped-empty CSC；超过两维、complex/sparse selector、N-D linear result、动态 source 继续失败关闭，并覆盖 alias/effect、source map、差分、runtime 拒错、fuzz、架构和独立场景性能预算
- [x] Matlab P0 sparse indexed assignment 纵切面：`SparseMutationPlan` 在 Semantic v20、MIR v26 和双目标 LIR v32 固化 linear/subscript assignment、linear/axis deletion、source/selection/replacement/result shape、dense/CSC replacement storage、scalar expansion、重复下标 last-write-wins 与 exact-zero erase；双目标直接合并 canonical CSC，支持 section 写入、静态扩容、合法 null deletion 与 self-alias，并覆盖 alias/effect、source map、差分、runtime 拒错、fuzz、架构和第二十七项独立性能预算
- [x] Matlab P0 sparse reshape 纵切面：`SparseReshapePlan` 在 Semantic v20、MIR v26 和双目标 LIR v32 固化 size-vector/dimension-list identity、单个 `[]` 推断轴、输入/请求/二维结果 shape 与 CSC storage；JavaScript/C++ 以 O(nnz + result-columns) 独立重映射列主序线性位置，不排序、不物化 dense source，并覆盖 alias/effect、纯 emitter、source map、负向语义、跨层损坏、差分、生成计划篡改拒错、fuzz、架构和第二十八项独立性能预算
- [x] Matlab P0 sparse matrix-product 纵切面：`MatrixStoragePolicy::sparse_csc_multiply` 在 Semantic v20、MIR v26 和双目标 LIR v32 固化两侧/结果 storage 与 shape；sparse×sparse 直接产生 canonical CSC，两类 mixed product 直接产生 dense，JavaScript/C++ 独立遍历非零项，并覆盖 source map、负向/跨层/LIR 损坏、差分、生成 runtime 篡改拒错、fuzz、架构和第二十九项独立性能预算
- [x] Matlab P0 sparse scalar-product 纵切面：`MatrixStoragePolicy::sparse_csc_scale` 在 Semantic v20、MIR v26 和双目标 LIR v32 固化 scalar 方向、CSC 结果 storage 与 shape；finite real/logical 非零因子由 JavaScript/C++ 独立 O(nnz) kernel 执行，零因子以 O(columns) 产生同 shape 空 CSC，并覆盖 source map、负向/跨层/LIR 损坏、差分、生成 nonfinite/overflow runtime 拒错、fuzz、架构和第三十项独立性能预算
- [x] Matlab P0 sparse element-wise 纵切面：独立 `SparseElementwisePlan` 在 Semantic v21、MIR v27 和双目标 LIR v33 固化 `.*` identity、五种 scalar/dense/CSC operand direction、静态 compatible-size 逐轴广播、canonical CSC 结果 storage 与 finite-real contract；JavaScript/C++ 独立 nonzero-driven kernel 不复用 matrix-product plan，并覆盖 source map、负向/跨层/LIR 损坏、双目标执行差分、生成 nonfinite/overflow/计划污染拒错、fuzz、架构和第三十一项独立性能预算
- [x] Matlab P0 静态零 extent sparse 收尾：constructor/transpose/index/mutation/reshape/matrix-product/scalar-product/element-wise 与方阵左右除均保留显式 shape；双目标 LIR v34 以可验证、可 dump 的 `runtime_shape_arguments` 固化 runtime 调用 ABI，JavaScript/C++ renderer 不读取 semantic storage/shape policy；差分 corpus、双目标生成执行、source map、计划篡改拒错、fuzz、架构与既有 sparse-solve 性能 workload 完成
- [x] Matlab P0 logical sparse storage 纵切面：`SparseValueDomain` 与 `SparseDuplicatePolicy` 在 Semantic v22、MIR v28 和双目标 LIR v35 固化 finite-real/logical CSC 与 numeric-sum/logical-any 重复项语义；`sparse(A)`、logical triplet、转置、scalar/linear/submatrix selection、indexed mutation/growth/deletion、reshape、`full`/`issparse`/`nnz` 均保持 logical class，JavaScript 使用带 `valueDomain` 的 tagged CSC，`cpp` 使用 `sparse_matrix<bool>`；目标 LIR 独立选择 runtime integer ABI/helper，renderer 只序列化，并覆盖 source map、逐层/LIR 损坏、双目标差分、生成 shape-plan 篡改拒错、fuzz、架构与第 32 项性能预算
- [x] Matlab P0 sparse logical operator 纵切面：独立 `SparseLogicalPlan` 在 Semantic v23、MIR v29 和双目标 LIR v36 固化 `~`/`&`/`|` identity、static rank-2 compatible-size shape、operand/result storage 与 preserve-sparse/materialize-dense policy；`~S`、至少一侧 sparse 的 `&` 及 sparse-sparse `|` 由 JavaScript/C++ nonzero-driven CSC kernel 执行，mixed sparse-dense/scalar `|` 按 Matlab storage 规则物化 dense；覆盖零 extent、broadcast、source map、逐层/目标 LIR/生成 ABI 损坏拒绝、双目标差分、fuzz、纯 Emitter 架构门禁和第 33 项独立性能预算
- [x] Matlab P0 sparse logical reduction 纵切面：`ReductionStoragePolicy` 在 Semantic v24、MIR v30 和双目标 LIR v37 固化 static real/logical rank-2 CSC `all`/`any` 的输入/结果 storage；默认维度、常量 `dim`/`vecdim`、`'all'`、高于 rank 的 no-op 维度与零 extent identity 均保持 Matlab shape/storage 规则，非标量结果为 canonical logical CSC、全维标量结果为 full logical；JavaScript/C++ 使用按需加载、彼此独立且不物化 dense source 的 O(nnz + output-extent) kernel，并覆盖逐层/目标 LIR/生成计划损坏拒绝、双目标差分、source map、fuzz、架构隔离和第 34 项独立性能预算
- [x] Matlab P0 sparse arithmetic 纵切面：独立 `SparseArithmeticPlan` 在 Semantic v25、MIR v31 和双目标 LIR v38 固化 static real/logical rank-2 CSC `+`/`-` 的 operation、compatible-size shape、两侧/结果 storage 与 preserve-sparse/materialize-dense policy；sparse-sparse 由 JavaScript/C++ 独立 CSC merge kernel 直接产生 canonical double CSC，mixed sparse-dense/scalar 直接产生 dense，支持 singleton row/column expansion 与零 extent；按需 runtime fragment、纯 Emitter、source map、逐层/目标 LIR/生成 ABI 损坏拒绝、双目标差分、nonfinite/overflow 拒绝、fuzz、架构隔离和第 35 项性能预算完成
- [x] Matlab P0 sparse matrix-power 纵切面：`MatrixStoragePolicy::sparse_csc_power` 与 `MatrixExponentPolicy::nonnegative_safe_integer` 在 Semantic v26、MIR v32 和双目标 LIR v39 固化 static real/logical square CSC 的输入/结果 storage、shape 与指数边界；JavaScript/C++ 以独立按需 runtime 执行重复平方法，零次幂直接产生同阶 canonical double CSC identity，logical base 提升为 double CSC；纯 Emitter、runtime shape/integer ABI、source map、逐层/目标 LIR/生成 exponent/storage/shape 计划损坏拒绝、双目标差分、动态非法指数、overflow、fuzz、架构隔离和第 36 项性能预算完成
- [x] Matlab P0 complex sparse storage 纵切面：`SparseValueDomain::finite_complex` 在 Semantic v27、MIR v33 和双目标 LIR v40 固化 complex CSC constructor、duplicate-sum、index/mutation/growth/reshape 与普通/共轭 transpose 合同；JavaScript tagged complex value 与 C++ `sparse_matrix<std::complex<double>>` 独立执行 canonical CSC 生命周期，显式 `RuntimeFeature::complex_sparse` 仅对实际 complex CSC 值装载组合 fragment，并覆盖无关 complex+real-sparse 裁剪、`full`/`issparse`/`nnz`、source map、逐层/LIR 损坏、双目标差分、nonfinite duplicate runtime 拒绝、fuzz、架构和第 37 项性能预算
- [x] Matlab P0 complex sparse arithmetic 纵切面：`SparseArithmeticPlan::value_domain` 在 Semantic v28、MIR v34 和双目标 LIR v41 固化 finite-real/finite-complex 结果域及第六个 runtime integer ABI；complex/real/logical CSC 的 `+`/`-`、双向 complex scalar、mixed dense、singleton row/column expansion、零 extent、exact-zero cancellation 与 nonfinite 拒绝由 JavaScript/C++ 独立执行，real-only 模块继续裁剪 complex runtime
- [x] Matlab P0 complex sparse matrix-product 纵切面：`MatrixStoragePolicy::sparse_csc_multiply` 与 `MatrixNumericDomain::complex` 在 Semantic v28、MIR v34 和双目标 LIR v42 固化 shape/storage/numeric-domain 及 runtime integer ABI；complex/real/logical CSC 与 dense 的三种矩阵组合、zero extent、canonical CSC、exact-zero cancellation 和 nonfinite 拒绝由 JavaScript/C++ 独立执行，product fragment 仅对实际 product 装载，sparse power 显式复用该依赖
- [x] Matlab→JavaScript 当前数组值语义边界：LIR v43 延续 `AssignmentValueForm::copy_array`、`ParameterPassing::value_copy` 和 `MutationOwnership::replace_with_result`，分别固定 dense name-to-name assignment、local-function 参数与 sparse indexed mutation 的 ownership；dense alias 在共享边界复制，sparse mutation 事务式返回并替换新 CSC 根值，旧 alias 不被 growth/erase/write 反向修改；arrays runtime 依赖闭包、LIR dump、计划损坏、直接/函数参数 alias 与双目标差分均已验证。一般动态 NDArray/view 的 copy-on-write 与 escape 优化仍由后续任务跟踪
...
- [x] Matlab 商业发布 P0-A 第一纵切面：函数/脚本裸 `return`、声明输出 SymbolId、双目标专属 return form、JavaScript 脚本控制拓扑、`disp`/`display` 单 character-vector command form、源码映射、差分、fuzz、性能与损坏计划拒绝完成
- [x] Matlab 商业发布 P0-A command 第二纵切面：独立 command scanner 覆盖多参数、单引号分组/双写引号、双引号保留和运算符空格消歧；受支持 local/builtin 调用通过 AST/HIR `ImplicitResultPolicy`、Analyzer `ans` 确定赋值、Semantic v30、MIR v36 与双目标 LIR v45 选择 value/discard form，多输出取首值、无输出保持旧 `ans`，C++ 不可表示或无法证明兼容的顺序重赋值/控制流结果类型在发射前失败关闭；source map、差分、fuzz、性能和逐层损坏拒绝完成
- [x] Matlab 商业发布 P0-A exception 纵切面：`try`/单一 `catch [exception]`、无绑定 handler、嵌套与最内层优先、`error(message)`/`error(identifier,message)`、`MException.identifier`/`message`、`rethrow`、确定赋值、异常 CFG edge/region、alias/effect、双目标独立 runtime、source map、差分、fuzz、性能与损坏事实拒绝完成
- [x] Matlab 商业发布 P0-A exception object 纵切面：`MException(identifier,message,...)`、`error` 的 identifier/message 与常用 scalar `sprintf` 格式、`throw`、`throwAsCaller`、`rethrow`、不可变 `addCause` 和 `getReport` 的 basic/extended/hyperlinks 选项进入 Semantic v34、MIR v40 与双目标 LIR v49 的强类型 operation/message-form/stack-policy；JavaScript/C++17 各自拥有按需异常 runtime、原始异常保留、嵌套 cause report、静态参数诊断、source map、差分、fuzz、运行期拒绝和第 45 项性能场景
- [x] Matlab 商业发布 P0-A 裸无参 local command 纵切面：frontend 仅把未被赋值、参数、结果、loop/catch/global/persistent binding 遮蔽的已知零输入 local function 规范化为 call；普通变量表达式保持原义，并进入 scanner、集成、差分、fuzz 和性能覆盖
- [ ] Matlab 商业发布 P0-A 剩余：依据 R2024a/R2024b grammar 补齐限定名/package/class command binding、外部 path/project 解析、`arguments` block 与产生式级 recovery/诊断；继续补齐公开 `MException.cause`/`stack`/`Correction` 对象属性、完整格式化合同及跨平台 stack frame 表示，未实现的 class/parallel/toolbox 产生式必须精确失败关闭
- [x] Matlab 商业发布 P0-B 第一纵切面：以 `IndexedReplacementContract` 将 scalar expansion、线性 numel、非 singleton shape 与动态 runtime dispatch 从 Analyzer 贯穿 Semantic v32、MIR v38 和双目标 LIR v47；未知形状函数参数的 `A(:)=B`/`A(:,:)=B` 不再误判为扩容，生成 runtime 会在任何写入前检查实际 shape，并覆盖列/行方向等价、matrix-shaped numeric selector 的线性数量、单元素数组扩展、捕获失败后的目标值不变、稳定拒错、source map、差分、fuzz 与性能场景
- [x] Matlab 商业发布 P0-B 第二纵切面：Semantic v33、MIR v39 与双目标 LIR v48 增加 `runtime` selector 和 `overwrite_or_grow` mutation 身份；未知形状 local-function 参数上的非全区段线性/多维赋值可按实际 scalar、numeric、logical 或 range selector 分派，覆盖覆盖写、间隙扩容、动态列删除和 N 维页扩容；JavaScript 先复制再增长、C++ 先在 staged value 上增长，RHS 不相容或计划损坏均在提交前失败，原目标值与 shape 保持不变；runtime selector 与 dense assignment 片段仅按实际 LIR 依赖装载，sparse mutation 产物不携带它们
- [ ] Matlab 商业发布 P0-B 剩余：建立可跨函数传播不可结构恢复零 extent 的统一动态 NDArray ABI，补齐一般 rank/shape/stride/layout、值语义、owner/view/copy-on-write 与 escape 边界；当前已支持的非全区段动态 assignment 仍要求嵌套数组结构能够恢复实际 rank/extent
- [ ] Matlab 商业发布 P0-C：优先交付 char/string、cell 与 struct 的类型、索引、赋值、展开和 Unicode/shape 语义，再进入 table/datetime 等高频对象
- [ ] Matlab 商业发布 P0-D：完成 script/function/nested workspace、anonymous/nested function、function handle、`nargin`/`nargout`/`varargin`/`varargout`、`global`/`persistent` 与捕获生命周期
- [ ] Matlab 商业发布 P0-E：建立常用 array/math/statistics/string/IO intrinsic catalog、项目级多文件/path/package 解析图和确定性 toolbox capability report
- [ ] Matlab 数值 P1：继续 sparse rectangular solve、complex sparse scalar-product/element-wise/power/solve 与动态 source-shape sparse，病态方阵特定解选择的精确 Matlab 对齐、其余 numeric class 和非整数矩阵幂；R2024 full symmetric-indefinite 已按当前 dense LU 行为处理，不把已移除的 LDL 路径列为兼容前提
...
- [ ] 完整 command/function 调用语法、character/string 数组转置和剩余字符串歧义解析；带参数的受支持 local/builtin command、`ans`、`disp`/`display` 单参数形式和未遮蔽零输入 local function 裸调用已完成，限定名/package/class 与外部 path/project 解析仍待交付
...
- [x] 静态 real/logical rank-2 CSC storage（包含零 extent）：R2024 全部 `sparse` 调用形式对相应值输入的 dense/CSC conversion/query/count、直接 canonical triplet construction、scalar expansion、numeric duplicate sum/cancellation、logical duplicate `any`、保持 class 的普通/共轭 sparse transpose、scalar/linear/submatrix indexing、indexed assignment/growth/deletion、size-vector/维度列表/单 `[]` 推断的列主序 reshape、含 `0×0` 系数和 shaped-empty operand/result 的稀疏实数方阵左右除，以及 sparse×sparse/sparse×dense/dense×sparse 矩阵乘法、双向 sparse/scalar 缩放、compatible-size sparse/dense/scalar `.*`、static compatible-size sparse `+`/`-`、static compatible-size logical `~`/`&`/`|`、shape/storage-preserving `all`/`any` 与非负 safe-integer sparse square power；sparse-sparse arithmetic、logical AND、sparse-sparse OR、非标量 logical reduction 与 sparse power 保持 CSC，mixed arithmetic/OR 物化 dense，全维 reduction 返回 full logical scalar
- [x] 静态 binary64 complex rank-2 CSC storage lifecycle：dense/triplet/copy constructor、duplicate complex sum/cancellation、普通/共轭 transpose、scalar/linear/submatrix index、indexed assignment/zero erase、列主序 reshape、`full`/`issparse`/`nnz` 与目标按需组合 runtime fragment
- [x] 静态 binary64 complex rank-2 CSC compatible-size `+`/`-`：complex/real/logical sparse promotion、双向 complex scalar、mixed dense materialization、canonical complex CSC、zero extent 与 exact-zero cancellation
- [x] 静态 binary64 complex rank-2 CSC matrix product：sparse×sparse 保持 canonical CSC，sparse×dense/dense×sparse 直接返回 dense，支持 real/logical promotion、zero extent、exact-zero cancellation 与按需 product runtime fragment
- [ ] sparse rectangular solve、complex sparse scalar-product/element-wise/power/solve 与动态 source-shape 语义，病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂、完整 numeric class，以及一般 NDArray 值语义
