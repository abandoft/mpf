...
| 工程门禁 | 213 项内部测试；70 个差分 case、185 条工具完整环境执行路径；Release/RelWithDebInfo 86 项、Debug 85 项 CTest；四语言 fuzz smoke、可选 libFuzzer、4 项生成 runtime 拒绝测试、发布脚本正/负契约、仅优化配置启用的版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [x] Matlab P0 rank-aware 矩形求解：修正欠定 `\`/`/` 为列主元 QR 基本最小二乘解，矩形数值秩亏继续计算并稳定警告；`MatrixRankPolicy` 贯穿 Semantic v9/MIR v14/双目标 LIR v20，方阵奇异仍保留独立 LU 失败边界
...
- [ ] 继续 Matlab P0：奇异方阵的 Matlab 数值/警告兼容策略、complex/numeric class，以及可跨函数传播 shape 的统一动态 NDArray ABI
...
- [ ] 奇异方阵求解的精确 Matlab 数值/警告行为、非整数矩阵幂、numeric class、complex、稀疏数组，以及一般 NDArray 值语义
