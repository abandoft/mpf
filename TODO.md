...
| 工程门禁 | 253 项内部测试；96 个差分 case、237 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 120 项 CTest；四语言 fuzz smoke、可选 libFuzzer、13 项生成 runtime 拒绝测试、发布脚本正/负契约、30 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
