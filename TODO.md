...
| 工程门禁 | 216 项内部测试；77 个差分 case、199 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 91 项 CTest；四语言 fuzz smoke、可选 libFuzzer、3 项生成 runtime 拒绝测试、发布脚本正/负契约、19 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛 85%，0.5.5 实测 90.40%（26,545/29,363）；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [x] Matlab P0 高级实数方阵分派：`classify_real_square` 贯穿 Semantic v12/MIR v17/双目标 LIR v23；full tridiagonal 使用相邻行部分主元紧凑 LU，非三对角 exact-symmetric positive-definite 使用 Cholesky，对称不定安全回退 dense LU，正/转置求解、条件 warning、左右除、source map、差分、fuzz 和性能均进入门禁
...
- [ ] 继续 Matlab P0：symmetric-indefinite 专用分解、Hermitian、sparse/complex solver dispatch 与病态方阵特定解选择的精确 Matlab 对齐、完整 numeric class，以及可跨函数传播 shape 的统一动态 NDArray ABI
...
- [ ] symmetric-indefinite 专用分解、Hermitian、sparse/complex solver dispatch、病态方阵特定解选择的精确 Matlab 对齐、非整数矩阵幂、完整 numeric class，以及一般 NDArray 值语义
