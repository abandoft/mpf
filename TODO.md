...
| 工程门禁 | 292 项内部测试；103 个差分 case、251 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 均为 145 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、31 项生成 runtime 拒绝测试、发布脚本正/负契约、36 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
