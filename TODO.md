...
本路线图记录 **0.7.9 开发中源码状态** 与后续交付目标。历史交付细节见
...
| Matlab control/function | 函数与脚本裸 `return`、通用 command syntax、`ans`、`disp`/`display`、`try`/单一 `catch [exception]` 和 exception object 已贯通名称/flow/MIR/双目标 LIR。`arguments` block 当前覆盖 positional input/output、显式 scalar/N 维 `double`/`logical`/char-vector ABI、按声明顺序的 default、输入/输出 validation、scalar expansion、row/column reshape 和 23 个无参数标准 validator；unsupported attribute/class/validator 在 frontend/Analyzer/target capability 边界失败关闭。 |
...
| 工程门禁 | 326 项内部测试；115 个差分 case、275 条工具完整环境执行路径；当前 dev preset 为 175 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、47 项生成 runtime 拒绝测试、3 项生成 C++ 编译、发布脚本正/负契约、46 项独立版本化通用及 Matlab 专项性能场景、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [x] Matlab 商业发布 P0-A `arguments` 首个纵切面：Matlab AST v5、Semantic v35、MIR v41 与双目标 LIR v51 保存 positional input/output declaration、formal representation rank、default、class/size boundary conversion、validator 与按需 runtime；R2019b input、R2022b output、R2020b validator expansion、R2024b row/column/matrix gate、top-to-bottom default、N 维 scalar expansion、row/column reshape、logical/char/empty/complex-storage validator 语义、R2024b variable-name contract、输出验证、source map、差分、生成 C++、runtime rejection、fuzz、性能和逐层损坏拒绝已进入门禁
- [ ] Matlab `arguments` P0 后续：支持 name-value 与 `Repeating` block、参数化标准/custom validator、numeric/logical 无显式 dimensions 的动态 rank/complexity ABI、完整 char/string/class conversion、sparse/nonsparse/file/folder validator 及 C++ 外部入口的动态 boundary adapter；这些完成前必须维持精确失败关闭，不能把当前纵切面表述成完整 R2024 `arguments` 兼容
- [ ] Matlab 商业发布 P0-A 剩余：依据 R2024a/R2024b grammar 补齐限定名/package/class command binding、外部 path/project 解析与产生式级 recovery/诊断；继续补齐公开 `MException.cause`/`stack`/`Correction` 对象属性、完整格式化合同及跨平台 stack frame 表示，未实现的 class/parallel/toolbox 产生式必须精确失败关闭
