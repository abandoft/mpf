...
| 工程门禁 | 156 项内部测试；48 个差分 case、134 条工具完整环境执行路径；59 项 CTest；fuzz smoke、可选 libFuzzer、版本化性能发布阈值、阶段报告；生产代码行覆盖率实测 89.01%（17502/19663），硬门槛 85% |
...
- [ ] 将 IIFE/closure/source-map segment representation 与结构化 import/export/chunk AST 从 renderer 前移到 LIR pass；当前支持的 statement/expression/operator/call/index、参数访问、临时值、scope declaration 与 module layout canonicalization 已完成
...
- [ ] 将尚未结构化的 ownership/copy-move、临时 lambda 和 runtime ABI node 从 renderer 前移到 LIR pass；当前支持的 statement/expression concrete form、optional/reference access、copy-section、temporary、函数 ABI、scope/declaration 与 translation-unit canonicalization 已完成
