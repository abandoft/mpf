...
本路线图记录 **0.7.3 当前开发基线** 与后续交付目标的真实状态。历史交付细节见
...
| IR 架构 | 四种语言使用编译期互不兼容的 PMR arena AST，并原子 lowering 到窄 HIR v2 与 revision-checked 稠密 semantic side table；名称、控制流、alias/effect 和 memory-dependence 分别由独立表持有。MIR v34 使用强类型稠密 expression/statement/instruction arena、显式 CFG、resident instruction 与 revision-bound attributes；Semantic v28→MIR v34→双目标 LIR v43 逐层验证 numeric class/complexity、dense/CSC array storage、matrix numeric domain/solve/condition/factorization/structure/storage/exponent policy、包含 finite-complex 的 sparse construction、arithmetic 与 matrix-product 值域、重复项策略、index/mutation/reshape/product/scale/power、独立 `SparseArithmeticPlan`/`SparseElementwisePlan`/`SparseLogicalPlan` 与带输入/结果 storage policy 的 `ReductionPlan`、storage region、call ownership/writeback、目标 runtime shape/integer 调用 ABI、函数输出/脚本退出形式和 source segment plan。JavaScript LIR 与 `cpp` LIR 各自完成 representation/runtime/module 规划，emitter 仅序列化，任一目标都不读取另一目标产物。 |
...
| Matlab control/command | 函数与脚本裸 `return` 已进入名称、flow、MIR 和双目标 LIR；函数早退保持声明的单/多输出，JavaScript 脚本使用碰撞安全的目标控制区，C++ 从生成入口返回；`disp`/`display` command syntax 支持恰好一个 character-vector 参数并保持引号规则 |
...
| 工程门禁 | 304 项内部测试；107 个差分 case、259 条工具完整环境执行路径；Debug/Release/RelWithDebInfo 均为 154 项普通 CTest，Release 流程另运行独立性能发布目标；四语言 fuzz smoke、可选 libFuzzer、36 项生成 runtime 拒绝测试、发布脚本正/负契约、40 项独立版本化通用及 Matlab 专项性能阈值、逐 pass/优化/内存依赖统计报告；生产代码行覆盖率硬门槛为 85%；Release 在标签 SHA 上复用七类 required workflow，门禁后才允许三平台候选测试/安装/消费/归档、来源证明和公开资产回验 |
...
- [ ] Matlab 商业发布 P0-A 剩余：依据 R2024a/R2024b grammar 补齐通用 command/function 调用、`try/catch`、`arguments` block 与产生式级 recovery/诊断；未实现的 class/parallel/toolbox 产生式必须精确失败关闭
...
- [ ] 完整 command/function 调用语法、character/string 数组转置和剩余字符串歧义解析；`disp`/`display` 的单 character-vector command form 已完成
