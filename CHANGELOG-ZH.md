...
- 为 0.4.8 增加 Matlab 数组与 tensor 专项性能门禁，版本化约束延迟、吞吐和生成产物大小。
- 生产代码行覆盖率实测为 89.52%（23,650/26,419），高于 85% 发布门槛。
- 更新 0.4.8 包消费示例与文档，并将内置 runtime 的供应链元数据修正为仓库实际使用的 MIT 许可证。
