...
|---|---|---|
| Matlab | `.m` | Scripts and local functions, conditionals/loops/scalar `switch`, element-wise N-D `~`/`&`/`|`, scalar short-circuit `&&`/`||`, array condition truthiness, matrices, shape-preserving `0×0` and zero-extent arrays, static N-D and local-function runtime-shape implicit expansion, array comparisons, static and runtime-sized `end`, ordered/repeated/empty numeric and linear/per-dimension logical selectors, vector/matrix/N-D growth and single-axis deletion, transpose, sections, `reshape`, two-dimensional matrix multiplication, structure-aware diagonal/triangular/pivoted-tridiagonal/symmetric-positive-definite/dense real square solves, rank-aware rectangular solves, square integer power, and multiple-output functions |
...
```cmake
find_package(mpf 0.5.5 EXACT CONFIG REQUIRED COMPONENTS core cpp)
